...
const { invalidateGraphCache } = require('../../lib/registry/impact/graph');
...
module.exports = async function run() {
    await suite('RegistryContext struct', async () => {
        await test('RegistryContext has all expected service properties', () => {
            const props = ['engine', 'relationships', 'capabilities', 'migrationLifecycle', 'projections', 'relationshipDiscovery'];
        ...
        await test('RegistryContext properties are lazy (getter-based)', () => {
            // Accessing twice should return the same object reference
    ...
    await suite('impact ctx injection', async () => {
        await test('analyze with mock ctx uses mock engine lookup', () => {
            // Reset graph so buildGraph(mockCtx) runs with mock relationships
            invalidateGraphCache();
            const r = impact.analyze(MOCK_ID, { depth: 1 }, mockCtx);
            invalidateGraphCache(); // restore for subsequent tests
    ...
    await suite('constraints ctx injection', async () => {
        await test('check uses mock engine for constitutional_gate', () => {
            // Mock engine returns MOCK_ENT (id MOCK-01, not ENT-000388).
            // ENT-000388 lookup returns null → gate rule should FAIL.
