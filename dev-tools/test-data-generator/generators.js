...
// ─── HEALTH RECORDS ───────────────────────────────────────────────────────────
function generateHealthRecords(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return { workouts: [], nutritionLogs: [], sleepLogs: [], bodyMeasurements: [] };
  const m = synthMeta(datasetId);

  const workoutData = [
    ['2026-06-28', 'Running', 35, 'Morning 5k'],
    ['2026-06-26', 'Weights', 50, 'Upper body — bench, rows, shoulder press'],
    ['2026-06-24', 'Cycling', 45, 'Outdoor ride'],
    ['2026-06-21', 'Running', 30, 'Easy recovery run'],
    ['2026-06-19', 'Weights', 55, 'Legs — squat, deadlift, lunges'],
    ['2026-06-17', 'HIIT',    25, 'Tabata intervals'],
    ['2026-06-14', 'Running', 40, '7k tempo run'],
    ['2026-06-12', 'Weights', 50, 'Push day'],
    ['2026-06-10', 'Yoga',    30, 'Morning flow'],
    ['2026-06-07', 'Running', 35, 'Parkrun 5k'],
    ['2026-06-05', 'Weights', 55, 'Pull day'],
    ['2026-06-03', 'Cycling', 60, 'Long ride 25km'],
    ['2026-05-31', 'Running', 30, 'Easy 5k'],
    ['2026-05-29', 'Weights', 50, 'Full body'],
    ['2026-05-27', 'HIIT',    20, 'Sprint intervals'],
    ['2026-05-24', 'Running', 40, 'Long run'],
    ['2026-05-22', 'Weights', 55, 'Upper body'],
    ['2026-05-20', 'Yoga',    45, 'Flexibility focus'],
    ['2026-05-17', 'Running', 35, 'Saturday 5k'],
    ['2026-05-15', 'Weights', 50, 'Legs day'],
    ['2026-05-13', 'Cycling', 40, 'City bike'],
    ['2026-05-10', 'Running', 30, 'Recovery jog'],
    ['2026-05-08', 'Weights', 55, 'Push/pull superset'],
    ['2026-05-06', 'HIIT',    25, 'Bodyweight circuit'],
    ['2026-05-03', 'Running', 40, '7k run'],
    ['2026-05-01', 'Weights', 50, 'Compound lifts'],
    ['2026-04-29', 'Yoga',    30, 'Evening wind-down'],
    ['2026-04-26', 'Running', 35, 'Trail run'],
    ['2026-04-24', 'Weights', 55, 'Full body strength'],
    ['2026-04-22', 'Cycling', 45, 'Morning ride'],
  ];
  const workouts = workoutData.map(([workout_date, type, duration_minutes, notes]) => ({
    type: `[SYNTHETIC] ${type}`, duration_minutes, notes: `[SYNTHETIC] ${notes}`, workout_date, ...m,
  }));

  const nutritionLogs = [
    { food_name: '[SYNTHETIC] Scrambled Eggs & Toast', calories: 420, protein_g: 22, carbs_g: 45, fat_g: 18, log_date: '2026-06-30', ...m },
    { food_name: '[SYNTHETIC] Chicken & Brown Rice',   calories: 580, protein_g: 48, carbs_g: 72, fat_g: 12, log_date: '2026-06-30', ...m },
    { food_name: '[SYNTHETIC] Protein Shake',          calories: 180, protein_g: 25, carbs_g: 8,  fat_g: 4,  log_date: '2026-06-30', ...m },
    { food_name: '[SYNTHETIC] Salmon & Vegetables',    calories: 520, protein_g: 38, carbs_g: 35, fat_g: 22, log_date: '2026-06-30', ...m },
  ];

  const sleepData = [
    ['2026-06-29', 7.5, 82, 'Good quality, woke once'],
    ['2026-06-28', 6.5, 68, 'Late night, groggy morning'],
    ['2026-06-27', 8.0, 90, 'Deep sleep, felt great'],
    ['2026-06-26', 7.0, 75, 'Normal night'],
    ['2026-06-25', 7.5, 80, 'Slight headache on waking'],
    ['2026-06-24', 6.0, 60, 'Restless, mind racing'],
    ['2026-06-23', 8.5, 88, 'Weekend catch-up sleep'],
  ];
  const sleepLogs = sleepData.map(([date, hours, quality_score, notes]) => ({
    date, hours, quality_score, notes: `[SYNTHETIC] ${notes}`, ...m,
  }));

  const bodyMeasurements = [
    { measured_at: '2026-06-01T08:00:00.000Z', weight_kg: 77.2, body_fat_pct: 14.8, notes: '[SYNTHETIC] Monthly weigh-in', ...m },
    { measured_at: '2026-05-01T08:00:00.000Z', weight_kg: 77.8, body_fat_pct: 15.2, notes: '[SYNTHETIC] Monthly weigh-in', ...m },
    { measured_at: '2026-04-01T08:00:00.000Z', weight_kg: 78.5, body_fat_pct: 15.8, notes: '[SYNTHETIC] Monthly weigh-in', ...m },
  ];

  return { workouts, nutritionLogs, sleepLogs, bodyMeasurements };
}

function generateMoodLogs(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
  return [
    { date: '2026-06-29', score: 8, ...m },
    { date: '2026-06-28', score: 7, ...m },
    { date: '2026-06-27', score: 9, ...m },
    { date: '2026-06-26', score: 7, ...m },
    { date: '2026-06-25', score: 6, ...m },
    { date: '2026-06-24', score: 8, ...m },
    { date: '2026-06-23', score: 8, ...m },
  ];
}

function generateSupplements(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
...
function generateJournalRecords(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return { journalEntries: [], habits: [] };
  const m = synthMeta(datasetId);

  const journalEntries = [
    { entry_text: '[SYNTHETIC] Good productive day. Finished the APEX synthetic data work and made solid progress on the pipeline. Feeling motivated about the project direction.', sentiment_score: 0.82, mood_score: 8, ...m },
    { entry_text: '[SYNTHETIC] Focused session this morning. Had a great workout before starting work. The business pipeline is moving well, three new leads came in from LinkedIn.', sentiment_score: 0.75, mood_score: 7, ...m },
    { entry_text: '[SYNTHETIC] Challenging day. Hit some technical blockers on the integration work but pushed through by evening. Exercise helped clear my head.', sentiment_score: 0.55, mood_score: 6, ...m },
    { entry_text: '[SYNTHETIC] Rest day. Caught up on reading and had a long meditation session. University assignment due next week — need to start planning it.', sentiment_score: 0.70, mood_score: 8, ...m },
    { entry_text: '[SYNTHETIC] High energy day. Ran 5k in the morning, had three productive client calls. The project is ahead of schedule. Feeling grateful.', sentiment_score: 0.90, mood_score: 9, ...m },
  ];

  const habits = [
    { habit_name: '[SYNTHETIC] Morning Meditation (10 min)', frequency: 'daily',  ...m },
    { habit_name: '[SYNTHETIC] Cold Shower',                 frequency: 'daily',  ...m },
    { habit_name: '[SYNTHETIC] Read 30 Minutes',             frequency: 'daily',  ...m },
    { habit_name: '[SYNTHETIC] Exercise',                    frequency: 'daily',  ...m },
    { habit_name: '[SYNTHETIC] No Added Sugar',              frequency: 'daily',  ...m },
    { habit_name: '[SYNTHETIC] Weekly Review',               frequency: 'weekly', ...m },
  ];

  return { journalEntries, habits };
}

function generateSpiritualRecords(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
  return [
...
function generateUniversityRecords(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return { modules: [], assignments: [], flashcards: [], readingList: [] };
  const m = synthMeta(datasetId);
  ...
  const assignments = [
    { title: '[SYNTHETIC] CS101 Essay — History of Computing',          module_id: null, due_date: '2026-07-15', weight_pct: 30, notes: '[SYNTHETIC] 2000 words required', ...m },
    { title: '[SYNTHETIC] CS201 Lab — Binary Search Implementation',    module_id: null, due_date: '2026-07-08', weight_pct: 20, notes: '[SYNTHETIC] Python or Java',       ...m },
    { title: '[SYNTHETIC] CS201 Coursework — Sorting Algorithms Report',module_id: null, due_date: '2026-07-22', weight_pct: 40, notes: '[SYNTHETIC] Benchmarking required', ...m },
    { title: '[SYNTHETIC] CS101 Quiz — Week 8 Theory',                  module_id: null, due_date: '2026-07-01', weight_pct: 10, notes: '[SYNTHETIC] Online, 45 mins',       ...m },
    { title: '[SYNTHETIC] CS150 Final Project — Portfolio Site',        module_id: null, due_date: '2026-06-01', weight_pct: 50, notes: '[SYNTHETIC] COMPLETED',             ...m },
    { title: '[SYNTHETIC] CS220 Database Design Project',               module_id: null, due_date: '2026-05-15', weight_pct: 50, notes: '[SYNTHETIC] COMPLETED — grade 68%', ...m },
  ];

  const flashcards = [
    { front: '[SYNTHETIC] What is Big O notation?',             back: '[SYNTHETIC] Upper bound on algorithm time complexity as input size grows.', module_id: null, next_review_at: '2026-06-25T00:00:00.000Z', ...m },
    { front: '[SYNTHETIC] Define a binary search tree',         back: '[SYNTHETIC] Tree where left child < node < right child, max 2 children.',   module_id: null, next_review_at: '2026-06-26T00:00:00.000Z', ...m },
    { front: '[SYNTHETIC] What is normalisation in databases?', back: '[SYNTHETIC] Organising DB to reduce redundancy. 1NF/2NF/3NF.',               module_id: null, next_review_at: '2026-06-27T00:00:00.000Z', ...m },
    { front: '[SYNTHETIC] TCP vs UDP difference?',              back: '[SYNTHETIC] TCP: reliable, ordered, connection-based. UDP: fast, unreliable.', module_id: null, next_review_at: '2026-06-28T00:00:00.000Z', ...m },
    { front: '[SYNTHETIC] What is O(log n) complexity?',        back: '[SYNTHETIC] Logarithmic time — halves problem size each step. Binary search.', module_id: null, next_review_at: '2026-06-29T00:00:00.000Z', ...m },
  ];
...
function generateContactRecords(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
  return [
    { name: '[SYNTHETIC] James Thornton',   email: 'james@synthclient.local',  phone: '+44 7700 900001', company: '[SYNTHETIC] Thornton Consulting Ltd',  ...m },
    { name: '[SYNTHETIC] Sarah Mitchell',   email: 'sarah@synthagency.local',  phone: '+44 7700 900002', company: '[SYNTHETIC] Mitchell Creative Agency',  ...m },
    { name: '[SYNTHETIC] David Okafor',     email: 'david@synthinvest.local',  phone: '+44 7700 900003', company: '[SYNTHETIC] Okafor Investments',        ...m },
    { name: '[SYNTHETIC] Emma Clarke',      email: 'emma@synthtech.local',     phone: '+44 7700 900004', company: '[SYNTHETIC] Clarke Tech Solutions',     ...m },
    { name: '[SYNTHETIC] Marcus Williams',  email: 'marcus@synthlaw.local',    phone: '+44 7700 900005', company: '[SYNTHETIC] Williams & Partners LLP',   ...m },
    { name: '[SYNTHETIC] Priya Sharma',     email: 'priya@synthgrowth.local',  phone: '+44 7700 900006', company: '[SYNTHETIC] Sharma Growth Partners',    ...m },
  ];
}

function generateSubscriptions(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
  return [
...
function generateInvestments(datasetId) {
  if (datasetId !== DATASET_IDS.TIER3) return [];
  const m = synthMeta(datasetId);
  return [
...
function generateApexTransactions(datasetId) {
...
function generateApexInvoices(datasetId) {
  const { invoices } = generateFinancialRecords(datasetId);
  return invoices.map(inv => ({
    title:       `[SYNTHETIC] Invoice ${inv.invoice_number}`,
    amount:      inv.amount,
    status:      inv.status,
...
module.exports = {
  ...
  generateMoodLogs,
  generateSupplements,
  generateJournalRecords,
  generateSpiritualRecords,
  generateUniversityRecords,
  generateContactRecords,
  generateSubscriptions,
  generateInvestments,
  generateApexTransactions,
  generateApexInvoices,
};
