...
async function validateTier3() {
  ...
  // Domain table counts
  const domainChecks = [
    { table: 'apex_transactions',       col: 'description', pattern: '[SYNTHETIC]%', expected: 24,  label: 'apex_transactions (synthetic)'       },
    { table: 'apex_invoices',           col: 'title',       pattern: '[SYNTHETIC]%', expected: 6,   label: 'apex_invoices (synthetic)'           },
    { table: 'apex_workouts',           col: 'notes',       pattern: '[SYNTHETIC]%', expected: 30,  label: 'apex_workouts (synthetic)'           },
    { table: 'apex_nutrition_log',      col: 'food_name',   pattern: '[SYNTHETIC]%', expected: 4,   label: 'apex_nutrition_log (synthetic)'      },
    { table: 'apex_sleep_log',          col: 'notes',       pattern: '[SYNTHETIC]%', expected: 7,   label: 'apex_sleep_log (synthetic)'          },
    { table: 'apex_body_measurements',  col: 'notes',       pattern: '[SYNTHETIC]%', expected: 3,   label: 'apex_body_measurements (synthetic)'  },
    { table: 'apex_supplements',        col: 'name',        pattern: '[SYNTHETIC]%', expected: 5,   label: 'apex_supplements (synthetic)'        },
    { table: 'apex_journal_entries',    col: 'entry_text',  pattern: '[SYNTHETIC]%', expected: 5,   label: 'apex_journal_entries (synthetic)'    },
    { table: 'apex_habits',             col: 'habit_name',  pattern: '[SYNTHETIC]%', expected: 6,   label: 'apex_habits (synthetic)'             },
    { table: 'apex_spiritual_sessions', col: 'notes',       pattern: '[SYNTHETIC]%', expected: 8,   label: 'apex_spiritual_sessions (synthetic)' },
    { table: 'apex_university_modules', col: 'name',        pattern: '[SYNTHETIC]%', expected: 4,   label: 'apex_university_modules (synthetic)' },
    { table: 'apex_university_assignments', col: 'title',   pattern: '[SYNTHETIC]%', expected: 6,   label: 'apex_university_assignments (synthetic)' },
    { table: 'apex_university_flashcards',  col: 'front',   pattern: '[SYNTHETIC]%', expected: 5,   label: 'apex_university_flashcards (synthetic)' },
    { table: 'apex_reading_list',       col: 'title',       pattern: '[SYNTHETIC]%', expected: 5,   label: 'apex_reading_list (synthetic)'       },
    { table: 'apex_contacts',           col: 'name',        pattern: '[SYNTHETIC]%', expected: 6,   label: 'apex_contacts (synthetic)'           },
    { table: 'apex_subscriptions',      col: 'name',        pattern: '[SYNTHETIC]%', expected: 5,   label: 'apex_subscriptions (synthetic)'      },
    { table: 'apex_investments',        col: 'name',        pattern: '[SYNTHETIC]%', expected: 3,   label: 'apex_investments (synthetic)'        },
  ];
  for (const { table, col, pattern, expected, label } of domainChecks) {
    const result = await _countSbRows(table, col, pattern);
    checks.push({ check: label, expected, actual: result.count, pass: result.count === expected, error: result.error });
  }
