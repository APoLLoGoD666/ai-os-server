...
// E1: 24h in-memory verdict cache keyed by question hash
const _verdictCache = new Map();
const VERDICT_TTL = 24 * 60 * 60 * 1000;
function _hashQuestion(entityId, question) {
    let h = 0;
    const s = `${entityId}:${(question || '').slice(0, 200)}`;
...
async function consultExecutive(entityId, question, context) {
    // E1: return cached verdict for identical questions within 24h
    const _qHash = _hashQuestion(entityId, question);
    const _cached = _verdictCache.get(_qHash);
    if (_cached && (Date.now() - _cached.ts) < VERDICT_TTL) return _cached.decision;
    ...
    _verdictCache.set(_qHash, { decision, ts: Date.now() });
