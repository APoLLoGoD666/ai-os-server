...
async function _runPolicySimulation(policyType, proposedChange, currentState) {
    ...
    // Founder anti-goal check — escalates risk if proposed change conflicts with founder constraints
    try {
        const _founder = require('../founder');
        const _antiCheck = await _founder.checkAntiGoals(proposedChange.title || '');
        if (!_antiCheck.clean) {
            if (_antiCheck.highest_severity === 'critical') riskEstimate = Math.min(0.95, riskEstimate + 0.55);
            else if (_antiCheck.highest_severity === 'high') riskEstimate = Math.min(0.90, riskEstimate + 0.25);
        }
    } catch {}
