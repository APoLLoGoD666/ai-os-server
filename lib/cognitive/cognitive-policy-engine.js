...
async function determine(spec, behaviorProfile, contextPack, options = {}) {
    ...
    let reasoningMode      = _selectReasoningMode(objective, complexity, incidents, episodes, riskScore);
    let planningMode       = _selectPlanningMode(complexity, incidents, decisions, riskScore);
    ...
    // Read cognitive_policy_settings and merge approved evolutions onto heuristic selections.
    // Only generic fallback modes are overrideable (ANALYTICAL, QUICK, STANDARD).
    // Condition-specific modes (ROOT_CAUSE, ADVERSARIAL, MULTI_HYPOTHESIS, etc.) are preserved.
    // DB unavailable → catch swallows, heuristic selections remain unchanged.
    try {
        const { data: _pRows } = await _sb().from('cognitive_policy_settings')
            .select('policy_name, policy_value').limit(50);
        const _ps = {};
        for (const r of (_pRows || [])) {
            if (r?.policy_value?.value !== undefined) _ps[r.policy_name] = r.policy_value.value;
        }
        if (_ps.default_reasoning_mode && REASONING_MODES.includes(_ps.default_reasoning_mode)
                && reasoningMode === 'ANALYTICAL') {
            reasoningMode = _ps.default_reasoning_mode;
        }
        const _dtp = { 1: 'QUICK', 2: 'STANDARD', 3: 'EVIDENCE_AWARE', 4: 'FULL' };
        const _dbPlan = _dtp[_ps.default_plan_depth];
        if (_dbPlan && PLANNING_MODES.includes(_dbPlan)
                && (planningMode === 'QUICK' || planningMode === 'STANDARD')) {
            planningMode = _dbPlan;
        }
    } catch (_) {}
    ...
    // Tasks 5+6: CLASS 2 sovereignty observability — log uncertainty; no execution block (no behaviour impact)
    const _uncertainty = parseFloat((1 - policy.confidence).toFixed(3));
    const _dominantSrc =
        /ROOT_CAUSE|ADVERSARIAL|CAUSAL|COUNTERFACTUAL/.test(reasoningMode) ? 'objective_keywords' :
        reasoningMode === 'MULTI_HYPOTHESIS' ? 'episodic_failures' :
        reasoningMode === 'DELIBERATE'       ? 'risk_score' :
        incidents.filter(i => i.status === 'open').length > 0 ? 'incidents' :
        episodes.length > 0                  ? 'episodic_history' : 'complexity_heuristic';
    console.log(`[DecisionSovereignty] class=CLASS_2 path=cognitive_policy control_plane_influence=false uncertainty=${_uncertainty} dominant_source=${_dominantSrc} divergence_index=N/A`);
...
function _selectReasoningMode(objective, complexity, incidents, episodes, riskScore) {
    ...
    // Security-specific objectives → adversarial thinking (word-boundary on 'auth' to avoid
    // matching 'authentication' in general refactoring tasks; root "vulnerabilit" catches
    // both singular and plural forms — holdout_reasoning_security revealed this gap)
    if (/security|\bauth\b|bypass|injection|vulnerabilit/i.test(obj)) return 'ADVERSARIAL';

    // Structural refactoring or architectural changes → deliberate reasoning
    if (/refactor|restructure|redesign|overhaul|rewrite/i.test(obj)) return 'DELIBERATE';
