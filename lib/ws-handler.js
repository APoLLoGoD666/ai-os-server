...
function init(server) {
    ...
    _wss.on('connection', (ws, req) => {
        ...
        const meta = { sessionId, connectedAt: new Date().toISOString(), channels: new Set(['system']),
                       humanId: null, role: 'master' };
        // V-11-A: extract identity from JWT cookie so per-user WS routing is possible.
        const jwtSecret = process.env.JWT_SECRET;
        if (jwtSecret) {
            const rawCookies = (req.headers.cookie || '').split(';').reduce((acc, c) => {
                const eq = c.indexOf('=');
                if (eq > 0) acc[c.slice(0, eq).trim()] = c.slice(eq + 1).trim();
                return acc;
            }, {});
            const cookieToken = rawCookies.apex_token;
            if (cookieToken) {
                try {
                    const pl = require('jsonwebtoken').verify(cookieToken, jwtSecret);
                    meta.humanId = (pl.sub && pl.sub !== 'apex-user') ? pl.sub : null;
                    meta.role    = pl.role || 'master';
                } catch (_) {}
            }
        }
    ...
    server.on('upgrade', (req, socket, head) => {
        ...
        } else if (urlPath === '/ws/viz') {
            require('./viz-broadcaster').handleUpgrade(req, socket, head);
