...
// L-DR-03: DOM-000001 not operational as deliberation participant at T3-12 bootstrap.
//           Status = NOT-OPERATIONAL when domOperRef is absent (backward-compatible).
//           RESOLVED by T4-05: when formDom000001Operationalization() produces domOperRef,
//           DOM-000001 participant status transitions to OPERATIONAL. NON-BLOCK.
...
const { formCausalModel }             = require('./rt11-bootstrap');        // T4-02
const { formDom000001Operationalization } = require('./dom000001-bootstrap'); // T4-05
...
// ── Element 3: Participants (L-DR-03 → RESOLVED by T4-05 when domOperRef present) ──
// A0 §3.12 R4 requires: Authority, Epistemic, Audit, Theory of Change, DOM-000001.
// T4-02: cmId parameter added — Theory of Change transitions BOOTSTRAP → ACTIVE.
// T4-05: domOperRef parameter added — DOM-000001 transitions NOT-OPERATIONAL → OPERATIONAL.
// Backward-compatible: cmId=undefined preserves BOOTSTRAP; domOperRef=undefined preserves
// NOT-OPERATIONAL (L-DR-03 original behaviour for standalone calls and prior tests).
function _buildDrParticipants(cmId, domOperRef) {
    return [
        {
            runtime: 'RT-11',
            role: 'Authority',
        ...
        {
            runtime: cmId ? 'RT-11' : 'CausalModel-BOOTSTRAP',
            role: 'Theory of Change',
            status: cmId ? 'ACTIVE' : 'BOOTSTRAP',
            note: cmId
                ? `CausalModel ${cmId} registered (T4-02 bootstrap; L-RT11-03)`
                : 'No CausalModel registered yet; Theory of Change pending extension',
        },
        {
            runtime: 'DOM-000001',
            role: 'Root Domain',
            // T4-05: domOperRef present → OPERATIONAL (L-DR-03 resolved by T4-05 bootstrap);
            // domOperRef absent → NOT-OPERATIONAL (backward-compatible, L-DR-03 applies)
            status: domOperRef ? 'OPERATIONAL' : 'NOT-OPERATIONAL',
            note:   domOperRef
                ? `T4-05: DOM-000001 operationalized (${domOperRef}); DomainProfile dp-DOM-000001 wired to decision pipeline`
                : 'L-DR-03: DOM-000001 not operational as deliberation participant at bootstrap',
        },
    ];
}
...
async function formDeliberationAndDecision({ cumId, cumVersion } = {}) {
    try {
        ...
        // T4-02: form CausalModel + AssumptionRegister before deliberation participants are built.
        // cmId used to replace CausalModel-BOOTSTRAP placeholder (L-RT11-05).
        // fire-and-forget: failure returns null; deliberation continues with BOOTSTRAP fallback.
        const rt11Result = await formCausalModel({ drId, cumVersionRef: cumVersion });
        const cmId = rt11Result?.cmId;

        // T4-05: operationalize DOM-000001 before deliberation participants are built.
        // domOperRef used to upgrade DOM-000001 participant status from NOT-OPERATIONAL → OPERATIONAL.
        // fire-and-forget: failure returns null; deliberation continues with NOT-OPERATIONAL fallback (L-DR-03).
        const dom000001Result = await formDom000001Operationalization({ drId, cdpId });
        const domOperRef = dom000001Result?.domOperRef;
        ...
        const drRecord = DeliberationRecord.create({
            ...
            // Element 3
            participants:            _buildDrParticipants(cmId, domOperRef),
            // Element 4
            evidence_used,
            // Element 5
