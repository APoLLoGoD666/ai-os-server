...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, CONDITIONAL HOLD, STRATEGIC ESCALATE). No JSON, no code blocks, no underscores in rationale.`,
...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, AUDIT REQUIRED, DEFER TO COUNCIL). No JSON, no code blocks, no underscores in rationale.`,
...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, HOLD PENDING REVIEW, ESCALATE BUDGET). No JSON, no code blocks, no underscores in rationale.`,
...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, CONDITIONAL DEPLOY, ESCALATE SECURITY). No JSON, no code blocks, no underscores in rationale.`,
...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, OPERATIONAL HOLD, ESCALATE PIPELINE). No JSON, no code blocks, no underscores in rationale.`,
    memoryAccess: [0, 6, 8],
    decisionRights: {
...
Respond with JSON only: {"choice": "DECISION PHRASE", "rationale": "One or two plain sentences.", "confidence": 0.0-1.0}
choice must be 2-5 UPPERCASE words (e.g. APPROVE, DEFER EXPERIMENT, GROWTH HOLD). No JSON, no code blocks, no underscores in rationale.`,
  ...
  cho: new ExecutiveEntity({
    id:   'cho',
    name: 'Chief Health Officer',
    systemPrompt: `You are the Chief Health Officer of APEX, Alex's personal AI civilization.
Your role: Maintain and optimize Alex's physical, mental, and cognitive health. Identify patterns, prevent burnout, and protect longevity.
You have read access to: health data, biometric trends, sleep patterns, activity logs.
Always return JSON: { choice, rationale, confidence, health_risk, recommended_action }`,
    memoryAccess: [0, 9],
    decisionRights: {
      can_approve:    ['health_routine_changes', 'recovery_protocols', 'wellness_initiatives'],
      cannot_approve: ['medical_decisions', 'pharmaceutical_changes'],
    },
    escalationRules: [
      { condition: (q) => /emergency|urgent|severe|critical\s*health/.test(q.toLowerCase()), name: 'health_emergency' },
    ],
  }),

  clo: new ExecutiveEntity({
    id:   'clo',
    name: 'Chief Legal Officer',
    systemPrompt: `You are the Chief Legal Officer of APEX, Alex's personal AI civilization.
Your role: Identify legal risks, ensure compliance, protect Alex's interests across contracts, IP, and regulatory matters.
You have read access to: compliance records, decision history, contractual obligations.
Always return JSON: { choice, rationale, confidence, legal_risk, compliance_status }`,
    memoryAccess: [0, 7, 9],
    decisionRights: {
      can_approve:    ['compliance_checks', 'contract_reviews', 'risk_assessments'],
      cannot_approve: ['legal_filings', 'court_submissions', 'regulatory_reporting'],
    },
    escalationRules: [
      { condition: (q) => /lawsuit|litigation|breach|regulatory|fine|penalty/.test(q.toLowerCase()), name: 'legal_action' },
    ],
  }),

  cro: new ExecutiveEntity({
    id:   'cro',
    name: 'Chief Risk Officer',
    systemPrompt: `You are the Chief Risk Officer of APEX, Alex's personal AI civilization.
Your role: Identify, assess, and mitigate risks across all domains. Protect the civilization from systemic failures.
You have read access to: risk registers, governance probes, certification status, audit logs.
Always return JSON: { choice, rationale, confidence, risk_level, mitigation_steps }`,
    memoryAccess: [0, 7, 9, 11],
    decisionRights: {
      can_approve:    ['risk_acceptance', 'mitigation_plans', 'security_exceptions_with_cso', 'audit_escalations'],
      cannot_approve: ['constitutional_changes', 'security_auth_changes'],
    },
    escalationRules: [
      { condition: (q) => /critical|breach|exploit|failure/.test(q.toLowerCase()), name: 'critical_risk' },
      { condition: (q, d) => d.risk_level === 'critical',                           name: 'critical_risk_rating' },
    ],
  }),
...
async function escalateToFounder(question, context, reason) {
  ...
  try {
    ...
    await alertCritical(
      `Founder Decision Required: ${reason}`,
