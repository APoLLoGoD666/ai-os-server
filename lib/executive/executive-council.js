    ...
    try {
        if (dispatch?.slug && dispatch?.action) {
            ...
            await _sb().from('apex_tasks').insert({
                ...
                description: recommendation ? recommendation.slice(0, 500) : null,
                status:      'awaiting_approval',
                metadata:    { type: 'council_dispatch', deliberationId, dispatch },
            });
            return;
        }
        ...
        if (recommendation && recommendation.length > 20) {
            ...
            await _sb().from('apex_tasks').insert({
                ...
                status:   'awaiting_approval',
                metadata: { type: 'council_recommendation', deliberationId, dispatch: null },
            });
        }
    } catch (e) {
