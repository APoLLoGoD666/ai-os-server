...
async function _createCouncilTask(recommendation, deliberationId) {
    try {
...
async function deliberate(question, context = {}) {
  ...
  setImmediate(() => _createCouncilTask(recommendation, deliberationId));
...
Return plain text, no JSON.`;
  ...
  let recommendation = '';
  try {
    const { result } = await runtime.execute({ tier: 'balanced', caller: 'executive-council', messages: [{ role: 'user', content: prompt }], maxTokens: 300 });
    recommendation = (result.content[0]?.text || '').trim();
  } catch {
