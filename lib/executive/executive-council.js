...
const _DOMAIN_PATTERNS = [
    ...
    { slug: 'marketing',    re: /marketing|brand|campaign|content|audience|social|instagram|ads|newsletter|seo|growth/i },
    { slug: 'comms',        re: /email|inbox|correspondence|client email|vendor|contractor|communications|triage/i },
...
// After a council decision, create an awaiting-approval task for the founder.
// Dedup strategy (runs unconditionally — not gated on dispatch slug):
//   1. Same dispatch slug + council type  → raise priority + bump re_escalations
//   2. Matching title prefix (first 60 chars)  → same
//   3. Matching task_title in metadata          → same
// Priority ladder on repeated escalation: standard → important → urgent → critical
const _PRI_TIERS = ['standard', 'important', 'urgent', 'critical'];
function _raisePriority(current) {
    const idx = _PRI_TIERS.indexOf(current || 'standard');
    return _PRI_TIERS[Math.min(idx + 1, _PRI_TIERS.length - 1)];
}
...
async function _createCouncilTask(taskTitle, recommendation, deliberationId, dispatch) {
    try {
        ...
        const [{ data: pending }, { data: recentDone }] = await Promise.all([
            _sb().from('apex_tasks')
                .select('id,title,status,metadata,created_at')
                .in('status', ['pending', 'awaiting_approval', 'pending_approval'])
                .order('created_at', { ascending: false })
                .limit(50),
            _sb().from('apex_tasks')
                .select('id,title,status,metadata,created_at')
                .in('status', ['completed', 'done', 'approved'])
                .gte('created_at', since24h)
                .order('created_at', { ascending: false })
                .limit(50),
        ]);

        const existing = [...(pending || []), ...(recentDone || [])];

        const normTitle    = _norm(title);
        const normTaskTitle = _norm(taskTitle);

        let dupe = null;

        // 1. Dispatch slug + title prefix match (slug-only was too broad — deduped unrelated requests)
        if (!dupe && dispatch?.slug && normTitle.length >= 20) {
            const prefix = normTitle.slice(0, 60);
            dupe = (existing || []).find(t =>
                t.metadata?.dispatch?.slug === dispatch.slug &&
                ['council_dispatch', 'council_recommendation'].includes(t.metadata?.type) &&
                _norm(t.title).slice(0, 60) === prefix
            );
        }

        // 2. Title prefix match (≥20 chars to avoid false positives on short generic titles)
        if (!dupe && normTitle.length >= 20) {
            const prefix = normTitle.slice(0, 60);
            dupe = (existing || []).find(t => _norm(t.title).slice(0, 60) === prefix);
        }

        // 3. task_title in metadata match
        if (!dupe && normTaskTitle.length >= 20) {
            const tprefix = normTaskTitle.slice(0, 60);
            dupe = (existing || []).find(t => _norm(t.metadata?.task_title).slice(0, 60) === tprefix);
        }

        // If the dupe is already completed/done, skip entirely — action was already taken
        if (dupe && (dupe.status === 'completed' || dupe.status === 'done' || dupe.status === 'approved')) {
            logger.debug('executive-council', 'deduped — already actioned within 24h', { id: dupe.id, status: dupe.status });
            return;
        }

        if (dupe) {
            const reEsc  = (dupe.metadata?.re_escalations || 0) + 1;
            const newPri = _raisePriority(dupe.metadata?.priority?.tier);
            const updMeta = {
                ...dupe.metadata,
                re_escalations: reEsc,
                last_escalated: new Date().toISOString(),
                priority: { tier: newPri },
            };
            await _sb().from('apex_tasks')
                .update({ metadata: updMeta, updated_at: new Date().toISOString() })
                .eq('id', dupe.id);
            logger.debug('executive-council', 'deduped — raised priority', {
                id: dupe.id, from: dupe.metadata?.priority?.tier || 'standard', to: newPri, re_escalations: reEsc,
            });
            return;
        }

        const newId = `COUNCIL-${Date.now().toString(36).toUpperCase()}`;
...
async function deliberate(question, context = {}) {
  ...
  // ── GAP-7 FIX: skip GATE task for quality evaluations that found no issues ──
  if (!(context.quality_check && !dispatch?.slug)) {
  ...
  return { deliberationId, recommendation, votes, consensusLevel, escalate, dispatch };
}
...
const _VALID_DISPATCH_SLUGS = ['finance', 'system', 'business', 'marketing', 'comms', 'uni', 'civilisation', 'file'];
...
Respond with ONLY valid JSON (no markdown, no prose outside it):
{
  ...
  "dispatch": {
    "slug": "<one of: finance, system, business, marketing, comms, uni, civilisation, file — or null if no domain action required>",
