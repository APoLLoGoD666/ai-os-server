...
// After a council decision, create an awaiting-approval task if the outcome is actionable.
async function _createCouncilTask(recommendation, deliberationId, dispatch) {
    try {
        if (!/approve|proceed|implement|execute|deploy|launch|commission|action/i.test(recommendation)) return;
        const newId = `COUNCIL-${String(Date.now()).slice(-6)}`;
        await _sb().from('apex_tasks').insert({
            id:       newId,
            title:    `[COUNCIL] ${recommendation.slice(0, 220)}`,
            status:   'awaiting_approval',
            metadata: { type: 'council_dispatch', deliberationId, dispatch: dispatch || null },
        });
    } catch {}
}
...
async function deliberate(question, context = {}) {
  ...
  // Create an awaiting-approval task for actionable council decisions (fire-and-forget)
  setImmediate(() => _createCouncilTask(recommendation, deliberationId, dispatch));
