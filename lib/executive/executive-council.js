...
async function _createCouncilTask(recommendation, deliberationId, dispatch) {
    try {
        if (dispatch?.slug && dispatch?.action) {
            // Specific domain dispatch — create directly without regex gate
            const newId = `COUNCIL-${String(Date.now()).slice(-6)}`;
            await _sb().from('apex_tasks').insert({
                id:       newId,
                title:    `[${dispatch.slug.toUpperCase()}] ${dispatch.action.slice(0, 200)}`,
                status:   'awaiting_approval',
                metadata: { type: 'council_dispatch', deliberationId, dispatch, recommendation: recommendation ? recommendation.slice(0, 500) : null },
            });
            return;
        }
        // Fallback: recommendation-level task when no domain dispatch was produced
        if (recommendation && recommendation.length > 20) {
            const newId = `COUNCIL-${String(Date.now()).slice(-6)}`;
            await _sb().from('apex_tasks').insert({
                id:       newId,
                title:    `[COUNCIL] ${recommendation.slice(0, 200)}`,
                status:   'awaiting_approval',
                metadata: { type: 'council_recommendation', deliberationId, dispatch: null, recommendation: recommendation.slice(0, 500) },
            });
        }
    } catch (e) {
...
async function deliberate(question, context = {}) {
  ...
  const { recommendation, consensusLevel, escalate, dispatch } = await _synthesize(question, votes, context);
  ...
  await _createCouncilTask(recommendation, deliberationId, dispatch);
...
{
  "recommendation": "<2-3 sentence decision: approve/reject/hold/conditional, key condition, who owns execution>",
  "dispatch": {
  ...
  let dispatch = null;
  try {
    const { result } = await runtime.execute({ tier: 'balanced', caller: 'executive-council', messages: [{ role: 'user', content: prompt }], maxTokens: 400 });
  ...
  return { recommendation, consensusLevel, escalate: anyEscalate || (avgConfidence < 0.45), dispatch };
}
