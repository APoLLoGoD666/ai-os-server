...
async function deliberate(question, context = {}) {
  ...
  // Update deliberation
  const discussion = votes.map(v => ({ entity: v.entityId, vote: v.vote, rationale: (v.rationale || '').slice(0, 300) }));
  await _sb()
    .from('executive_deliberations')
    .update({
      discussion,
...
async function _synthesize(question, votes, context) {
  ...
  const voteSummary = votes.map(v =>
    `${v.entityId.toUpperCase()}: ${v.vote} (confidence=${v.confidence.toFixed(2)}) — ${(v.rationale || '').slice(0, 200)}`
  ).join('\n');
