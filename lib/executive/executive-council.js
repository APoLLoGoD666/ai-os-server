...
// Dedup: if a pending/awaiting council task already exists for the same dispatch slug,
// increment its re_escalations counter instead of creating a duplicate.
async function _createCouncilTask(taskTitle, recommendation, deliberationId, dispatch) {
    try {
        ...
        if (dispatch?.slug) {
            ...
            if (dupe) {
                const updMeta = { ...dupe.metadata, re_escalations: ((dupe.metadata?.re_escalations || 0) + 1), last_escalated: new Date().toISOString() };
                await _sb().from('apex_tasks').update({ metadata: updMeta, updated_at: new Date().toISOString() }).eq('id', dupe.id);
                logger.debug('executive-council', 'deduped council task — bumped re_escalations', { id: dupe.id, slug: dispatch.slug });
                return;
            }
        }
