...
function _matches(role, triggers, ctx) {
    switch (role) {
        ...
        case 'cgo': {
            ...
            if (!cond) {
                if (role === 'cso' && desc && /strategy|roadmap|vision|priorit|initiative|direction/i.test(desc)) return true;
                if (role === 'cio' && desc && /memory.policy|cognitive.policy|knowledge.decay|learning.rate|retention|benchmark/i.test(desc)) return true;
                if (role === 'cgo' && desc && /new.feature|opportunity|experiment|capability|grow|expand/i.test(desc)) return true;
                return false;
            }
        ...
        case 'cro': {
            ...
            const cond = triggers.condition || '';
            const desc = (ctx.taskDescription || '').toLowerCase();
            if (!cond) return ctx.complexity === 'critical' || (desc && /risk|threat|vulnerabilit|security.breach|failure.mode|incident|outage/i.test(desc));
            if (cond === 'critical_complexity') return ctx.complexity === 'critical';
            if (cond.startsWith('complexity_eq_')) return ctx.complexity === cond.replace('complexity_eq_', '');
            return false;
        }
        case 'cho': {
            const cond = triggers.condition || '';
            const desc = (ctx.taskDescription || '').toLowerCase();
            if (!cond) return desc ? /health|recovery|burnout|cognitive.load|wellbeing|fatigue/i.test(desc) : false;
            if (cond === 'critical_complexity') return ctx.complexity === 'critical';
            return false;
        }
        case 'clo': {
            const cond = triggers.condition || '';
            const desc = (ctx.taskDescription || '').toLowerCase();
            if (!cond) return desc ? /legal|compliance|contract|regulation|gdpr|privacy|audit/i.test(desc) : false;
            if (cond === 'critical_complexity') return ctx.complexity === 'critical';
            return false;
        }
        default:
...
function _hardcodedFallback(ctx) {
    ...
    const desc = (ctx.taskDescription || '').toLowerCase();
    if (desc && /strategy|roadmap|vision|priorit|initiative|direction/i.test(desc)) out.push('cso');
    if (desc && /memory.policy|cognitive.policy|knowledge.decay|learning.rate|retention/i.test(desc)) out.push('cio');
    if (desc && /new.feature|opportunity|experiment|capability|grow|expand/i.test(desc)) out.push('cgo');
    if (ctx.complexity === 'critical' || (desc && /risk|threat|vulnerabilit|security.breach|failure.mode|incident|outage/i.test(desc))) out.push('cro');
    if (desc && /health|recovery|burnout|cognitive.load|wellbeing|fatigue/i.test(desc)) out.push('cho');
    if (desc && /legal|compliance|contract|regulation|gdpr|privacy|audit/i.test(desc)) out.push('clo');
    return out;
}
