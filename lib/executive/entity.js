...
class ExecutiveEntity {
  ...
  async decide(question, context = {}) {
    ...
    const _execSystem = [
      _getManifest(),
      this.systemPrompt || 'You are APEX, an executive decision-making entity.',
      `Role: ${this.name}`,
      this.decisionRights ? `Decision authority: ${JSON.stringify(this.decisionRights).slice(0, 200)}` : '',
    ].filter(Boolean).join('\n\n');
    const { result } = await runtime.execute({
      tier:     'balanced',
      caller:   `executive.${this.id}`,
      system:   _execSystem,
      messages: [{ role: 'user', content: question }],
      maxTokens: 1024,
    });

    const decision     = this._parseDecision(result.content[0]?.text || '');
  ...
  _parseDecision(content) {
    ...
    if (parsed) {
      parsed.choice    = this._cleanDecisionText(parsed.choice,    40);
      parsed.rationale = this._cleanDecisionText(parsed.rationale, 300);
      return parsed;
    }
    return { choice: this._cleanDecisionText(content, 40), rationale: '', confidence: 0.7 };
  }

  _cleanDecisionText(text, maxLen = 300) {
    if (!text || typeof text !== 'string') return '';
    // Strip code fences and JSON blobs
    text = text.replace(/```[\w]*\n?[\s\S]*?```/g, '').trim();
    if (text.startsWith('{') || text.startsWith('[')) {
      try {
        const inner = JSON.parse(text);
        text = inner.choice || inner.decision || inner.summary || inner.rationale || '';
      } catch {
        text = text.replace(/[{}\[\]"]/g, ' ').replace(/\s+/g, ' ').trim();
      }
    }
    // Strip underscores used as separators, normalise whitespace
