...
async function runWeeklySession() {
  ...
  if (decisions.length) {
    ...
    // ── GAP-3 FIX: convert decisions into awaiting-approval tasks at the GATE ──
    const taskRows = decisions.map((d, i) => ({
      id:       `CDEC-${Date.now()}-${i}`,
      title:    d.description.slice(0, 200),
      ...
      metadata: {
        ...
        dispatch:       { slug: topic.dispatch_hint, action: d.description.slice(0, 300) },
        recommendation: (result.recommendation || '').slice(0, 400),
      },
    }));
    await _sb().from('apex_tasks').insert(taskRows).catch(e =>
      log.warn('council-session', 'Failed to create decision tasks', { error: e.message })
    );
  }
