...
function _buildGraph() {
    ...
    for (const rel of _rels) {
        ...
        _graph.get(rel.from).push(Object.freeze({ to: rel.to, type: rel.type, label: rel.label || '', strength: rel.strength || 'optional', reason: rel.reason || '' }));
        // Reverse edge (type prefixed with ~) for inbound traversal
        if (!_graph.has(rel.to)) _graph.set(rel.to, []);
        _graph.get(rel.to).push(Object.freeze({ to: rel.from, type: `~${rel.type}`, label: rel.label || '', strength: rel.strength || 'optional', reason: rel.reason || '' }));
    }
}
...
function add(from, to, type, label = '', strength = 'optional', reason = '') {
    ...
    _graph = null;
    EventBus.emit(EVENTS.EDGE_ADDED, { from, to, type });
}
...
function graph(startId, maxDepth = 2) {
    _ensure();
    return require('../graph-traversal').neighbours(startId, _graph, maxDepth);
}
