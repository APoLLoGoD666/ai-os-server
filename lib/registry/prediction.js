 ...
 */
function simulateEntityChange(entityId, proposedChanges = {}) {
    const t0     = Date.now();
    ...
    const constr = require('./constraints');
    ...
    const currentRisk        = impact.quickRisk(entityId);
    ...
    // ── Relationship impact ───────────────────────────────────────────────────
    // Relationships that become semantically invalid (e.g. entity moves family)
    const outgoing = rels.relationsOf(entityId);
    const incoming = rels.reverseRelationsOf(entityId);

    // Orphan check: if entity is removed (status → DEPRECATED/REMOVED) flag dependents
    ...
    const atriskDependents = willBecomeInactive ? incoming.map(r => {
        const src = engine.lookup(r.from);
        return { id: r.from, name: src?.name || null, rel_type: r.type, risk: 'Depends on entity being deactivated' };
    }) : [];

    // ── Blast radius (unchanged — impact analysis reads from Registry, not our patch) ──
    const blastReport   = impact.analyze(entityId, { depth: 3, direction: 'upstream' });
    const blastRadius   = blastReport?.blast_radius || { direct: 0, transitive: 0, total: 0 };

    // ── Constraint re-evaluation ──────────────────────────────────────────────
    // Run static constraints against the Registry as-is.
    // We can't patch the Registry in-memory, so we flag constraints that WILL be
    // violated by the proposed change based on field-level reasoning.
    const newViolations = [];
