...
// ── Static evaluators ─────────────────────────────────────────────────────────
// Each evaluator accepts an optional ProjectedGraph.
// When graph is provided, entity lookups use graph.lookup() / graph.all() —
// the live engine is never consulted for those calls.

function constitutional_gate_healthy(graph) {
    const _lookup = graph ? id => graph.lookup(id) : require('./engine').lookup.bind(require('./engine'));
...
function no_orphaned_relationships(graph) {
    const _lookup    = graph ? id => graph.lookup(id) : require('./engine').lookup.bind(require('./engine'));
...
function governed_migration_approved(_graph) {
    // Migration headers are filesystem data — entity state is irrelevant
...
function active_service_monitored(graph) {
    const _all       = graph ? () => graph.all() : () => require('./engine').all();
...
function gov_entity_documented(graph) {
    const _all       = graph ? () => graph.all() : () => require('./engine').all();
    const proj       = require('./projections');
    const violations = [];
    for (const e of _all()) {
...
function active_file_in_repo(graph) {
    const _all       = graph ? () => graph.all() : () => require('./engine').all();
    const proj       = require('./projections');
...
function high_impact_entity_documented(graph) {
    const _all       = graph ? () => graph.all() : () => require('./engine').all();
    const proj       = require('./projections');
 ...
 *
 * @param {{ full?: boolean, graph?: ProjectedGraph }} opts
