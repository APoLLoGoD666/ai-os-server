...
function runScenario(opts = {}) {
    ...
    const entityImpacts = changes.map(c => entityImpact(c.entity_id, c.proposed, graph));

    // Step 2: Capability degradation + projected capability status
    const affectedIds   = entityImpacts.filter(e => e.ok).map(e => e.entity_id);
    const capImpacts    = capabilityImpacts(affectedIds, graph);
    ...
    const executive = buildExecutive(urgency, confidence, rationale, entityImpacts, capImpacts, constraintResult, changes);
