...
function _buildSnapshotPayload() {
    ...
    return {
        ...
        high_risk_entities: highRisk.slice(0, 20),
        snapshot_data: {
...
async function diffSnapshots(id1, id2) {
    ...
    try {
        ...
        const aRiskIds = new Set((a.high_risk_entities || []).map(e => e.id));
        const bRiskIds = new Set((b.high_risk_entities || []).map(e => e.id));
        const newRisk  = (b.high_risk_entities || []).filter(e => !aRiskIds.has(e.id));
        const resolvedRisk = (a.high_risk_entities || []).filter(e => !bRiskIds.has(e.id));
