...
// Capability role signal constants
// Uses registry status field (CANONICAL_FACT) — independent from projection signals.
const HEALTHY_STATUSES_CAP = new Set(['ACTIVE', 'Production', 'Active', 'production', 'PRODUCTION']);
const DOWN_STATUSES_CAP    = new Set(['INACTIVE', 'DECOMMISSIONED', 'REMOVED', 'DEPRECATED', 'Decommissioned']);
const CAP_WEIGHT           = { CRITICAL: 0.12, HIGH: 0.08, MEDIUM: 0.05, LOW: 0.02, MINIMAL: 0.01 };
...
function compute(entity, projections, opts = {}) {
    ...
    // Capability role signal — a DEPRECATED entity that is a required dep of a CRITICAL
    // capability gets a 0.12-weight penalty dragging its score toward 0.
    const capRole = opts.capabilityRole;
    if (capRole && capRole.affected_count > 0 && capRole.worst_severity) {
        const statusValue = HEALTHY_STATUSES_CAP.has(entity.status) ? 1.0
                          : DOWN_STATUSES_CAP.has(entity.status)    ? 0.0
                          : 0.5;
        const weight = CAP_WEIGHT[capRole.worst_severity] || 0.02;
        signals.push({
            source:     'capability_role',
            weight,
            value:      statusValue,
            status:     `${capRole.worst_severity}_role`,
            confidence: 0.90,
        });
    }
