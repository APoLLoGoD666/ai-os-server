...
// ── Shared timestamp ─────────────────────────────────────────────────────────
// All edges in a single discovery run share the same observation timestamp.

const _RUN_TS = new Date().toISOString();
    ...
    for (const m of ml.scanMigrations()) {
        ...
        for (const toId of m.entRefs) {
            edges.push({
                ...
                observed_by:  'migration-header-scan',
                derived_from: relPath,
                strength:     'required',
                reason:       'schema',
                first_seen:   _RUN_TS,
