...
function _scanDir(dir, engine, edges) {
    ...
    for (const f of entries) {
        ...
        if (stat.isDirectory()) {
            if (!SKIP_DIRS.has(f)) _scanDir(full, engine, edges);
            continue;
        }
...
function jsPass(ctx) {
    ...
    const edges = [];
    _scanDir(SCRIPTS_ROOT, ctx.engine, edges);
    return edges;
}

const jsPlugin = {
    name:        'js',
    description: 'Scans JS/TS source files for require() and import statements to emit depends_on edges',
    fileTypes:   ['js', 'ts'],
    confidence:  0.9,
    discover:    jsPass,
...
module.exports = { jsPass, jsPlugin };
