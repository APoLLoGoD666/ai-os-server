...
/**
 * Compute the current operational status of a capability.
 ...
 */
function statusOf(capabilityId) {
    ...
    for (const dep of cap.depends_on) {
        const entity = engine.lookup(dep.id);
        ...
        if (health === 'down') {
            const impact = dep.strength === 'required' ? 'DOWN'
                         ...
                         : 'OPERATIONAL';   // fallback failing alone doesn't degrade
 ...
 */
function fullReport() {
    const capabilities = Object.keys(CAPS).map(id => statusOf(id));
    const summary = {
