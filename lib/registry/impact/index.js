...
function _getMaps(graph) {
    if (graph && graph.hasEdgePatches) return buildLocalAdjacency(graph);
...
function analyze(entityId, opts = {}) {
    ...
    const { forwardMap, backwardMap } = _getMaps(graph);
    ...
    return {
        ...
        capabilities:     require('../capabilities').degradationFrom(entityId),
        affected: {
