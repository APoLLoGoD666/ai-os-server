...
const { buildLocalAdjacency } = require('./graph');
const { classifyRisk }        = require('./risk');
const { DocIndex }            = require('./docs');
...
function _getMaps(projGraph, ctx) {
    ...
    return { forwardMap: ctx.graph.forward(), backwardMap: ctx.graph.backward() };
}
...
function _traverse(entityId, direction, depth, forwardMap, backwardMap) {
    ...
    if (direction === 'upstream' || direction === 'both') {
        const r = GraphTraversal.traverse(entityId, depth, backwardMap);
        upNodes = r.nodes; upEdges = r.edges; upConf = r.nodeConf;
    }
    if (direction === 'downstream' || direction === 'both') {
        const r = GraphTraversal.traverse(entityId, depth, forwardMap);
...
function analyze(entityId, opts = {}, ctx = RegistryContext) {
    ...
    return {
        ...
        affected: {
            ...
            docs: DocIndex.refsForEntities([entityId, ...allAffectedIds]),
        },
...
function quickRisk(entityId, projGraph, ctx = RegistryContext) {
    ...
    const directIds      = (ctx.graph.backward().get(entityId) || []).map(e => e.from);
    const directEntities = directIds.map(id => _lookup(id)).filter(Boolean);
