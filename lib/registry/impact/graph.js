...
function buildGraph(ctx = RegistryContext) {
    if (_forward) return;
    _forward  = new Map();
    _backward = new Map();
...
// Called by relationships.add() after edges are mutated post-build.
// Forces the next analyze() call to rebuild from the updated edge list.
function invalidateGraphCache() { _forward = null; _backward = null; }

module.exports = { buildGraph, bfs, buildLocalAdjacency, getForward, getBackward, invalidateGraphCache };
