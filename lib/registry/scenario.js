...
function _buildRationale(urgency, entityImpacts, capImpacts, constraintResult) {
    ...
    if (criticalCaps.length) parts.push(`Critical capability degradation: ${criticalCaps.map(c => c.capability_name || c.capability_id).join(', ')}.`);

    const highCaps = capImpacts.filter(c => c.severity === 'HIGH');
    if (highCaps.length) parts.push(`High-severity capability impact: ${highCaps.map(c => c.capability_name || c.capability_id).join(', ')}.`);
