...
function computeState(entity) {
    ...
    // Capability role — independent signal fed into health scoring
    const capRole = caps.degradationFrom(entity.id);
    ...
    return {
        ...
        // Evidence-based health (now capability-aware)
        health:          health.label,
        health_score:    health.score,
        confidence:      health.confidence,
        evidence:        health.evidence,

        // Capability role (null if entity has no named capability dependencies)
...
/**
 * Persist a computed state to entity_state (upsert).
 * Also writes a history row to entity_state_history when the health label changes.
 * Fail-soft — persistence errors never crash the twin.
 */
async function persistState(state) {
    const sb = _getSb();
    if (!sb) return false;
    try {
        // Read current persisted label to detect changes
        ...
        // Append to history only on label transition (keeps table lean)
        if (labelChanged) {
