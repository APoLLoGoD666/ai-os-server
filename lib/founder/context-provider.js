...
async function getAlignmentGuidanceForPrompt(taskDescription = '') {
  ...
  // Append TF-IDF vault notes most relevant to this task description
  try {
    const obsidianMemory = require('../../agent-system/obsidian-memory');
    const vaultCtx = obsidianMemory.getVaultContext(taskDescription);
    if (vaultCtx) return guidance + '\n\n' + vaultCtx;
  } catch {}
  return guidance;
}
