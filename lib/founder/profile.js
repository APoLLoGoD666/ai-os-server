...
// RT-01 ActorProfile wiring (W2-12)
const { ActorProfile } = require('../constitutional-types/identity-record');
...
async function load(force = false) {
  ...
  // RT-01 ActorProfile — fire-and-forget (W2-12; CONSTITUTIONAL WIRING PATTERN V1.0)
  const _displayName = profile.identity?.name || profile.identity?.display_name;
  setImmediate(async () => {
    try {
      const record = ActorProfile.create({
        actor_id:      'FOUNDER-APEX',
        actor_type:    'HUMAN',
        display_name:  _displayName || 'APEX Founder',
        registered_at: new Date().toISOString(),
        status:        'ACTIVE',
      });
      await constitutionalStore.write(record);
    } catch (err) {
      console.error('[constitutional-record] ActorProfile failed:', err?.message);
    }
  });
