...
// RT-04 ConstitutionalComplianceAttestation wiring (W2-08)
const { ConstitutionalComplianceAttestation } = require('../constitutional-types/audit-record');
...
function createGovernanceAttestation() {
    ...
    // RT-04 ConstitutionalComplianceAttestation — fire-and-forget (W2-08; CONSTITUTIONAL WIRING PATTERN V1.0)
    const _match = match;
    const _eHash = compiled.contractHash;
    const _sHash = sourceHash;
    setImmediate(async () => {
        try {
            const _ts = new Date().toISOString();
            const record = ConstitutionalComplianceAttestation.create({
                attestation_id:             `ATTEST-governance-${Date.now()}`,
                target_identifier:          'APEX-CONSTITUTION-v1.0/governance-contract',
                certification_period_start: _ts,
                certification_period_end:   _ts,
                attestation_determination:  _match ? 'PASS' : 'FAIL',
                evidence_basis:             _eHash,
                issuing_auditor_signature:  _sHash,
                attest_timestamp:           _ts,
            });
            await constitutionalStore.write(record);
        } catch (err) {
            console.error('[constitutional-record] ConstitutionalComplianceAttestation failed:', err?.message);
        }
    });
