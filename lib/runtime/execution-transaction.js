...
const feedback         = require('./lattice-feedback-loop');
...
function _internalAbort(tx, type, stage, reason, context) {
    ...
    const _a_tx = tx; const _a_stage = stage; const _a_reason = reason; const _a_ctx = context;
    setImmediate(async () => {
        try {
            const record = RejectionRecord.create({
                rejection_id:         `RJCT-${_a_tx.txId}-${Date.now()}`,
                gate:                 _STAGE_TO_GATE[_a_stage] || 'GATE-5',
                failed_condition:     _a_reason || _a_stage,
                actor_identity:       _a_tx.userId || 'anonymous',
                operation_type:       `${_a_tx.method}:${_a_tx.path}`,
                operation_identifier: _a_tx.txId,
                timestamp:            new Date().toISOString(),
                constitutional_refs:  _a_ctx?.risks,
            });
            await constitutionalStore.write(record);
        } catch (err) {
            console.error('[constitutional-record] RejectionRecord failed:', err?.message);
        }
    });
}
...
function begin(req = {}, opts = {}) {
    ...
    if (!concStage.passed) {
        ...
        tx.preflight.durationMs = Date.now() - t0;
        // Constitutional wiring — fire-and-forget (CONSTITUTIONAL WIRING PATTERN v1.0)
        const _c_txId = txId; const _c_reason = concStage.reason; const _c_userId = tx.userId; const _c_op = `${method}:${path}`;
        setImmediate(async () => {
            try {
                const record = RejectionRecord.create({
                    rejection_id:         `RJCT-${_c_txId}-CONC`,
                    gate:                 'GATE-2',
                    failed_condition:     _c_reason,
                    actor_identity:       _c_userId || 'anonymous',
                    operation_type:       _c_op,
                    operation_identifier: _c_txId,
                    timestamp:            new Date().toISOString(),
                });
                await constitutionalStore.write(record);
            } catch (err) {
                console.error('[constitutional-record] RejectionRecord (concurrency) failed:', err?.message);
            }
        });
...
function finalize(txId, result = null) {
    ...
    feedback.record(tx);
    healthSignal.record(tx);

    // Constitutional wiring — fire-and-forget (CONSTITUTIONAL WIRING PATTERN v1.0)
    const _f_txId = txId; const _f_tx = tx;
    const _f_constStg = tx.preflight.stages.find(s => s.name === 'CONSTITUTION');
    const _f_authRef = (_f_constStg?.data?.auditTrail?.[0]) ? String(_f_constStg.data.auditTrail[0]) : 'PETL-CONSTITUTION-PASS';
    setImmediate(async () => {
        try {
            const record = AccountabilityRecord.create({
                accountability_id:     `ACC-${_f_txId}`,
                actor_identity:        _f_tx.userId || 'anonymous',
                authority_ref:         _f_authRef,
                operation_type:        `${_f_tx.method}:${_f_tx.path}`,
                operation_identifier:  _f_txId,
                timestamp:             _f_tx.finalizedAt || new Date().toISOString(),
                constitutional_effect: `${_f_tx.method} ${_f_tx.path} completed (FINALIZED)`,
                stage8_commit_ref:     _f_txId,
            });
            await constitutionalStore.write(record);
        } catch (err) {
            console.error('[constitutional-record] AccountabilityRecord failed:', err?.message);
        }
    });
...
// Constitutional wiring — KernelOperationManifest (era-level, one-time at module init)
// CONSTITUTIONAL WIRING PATTERN v1.0 — W2-02
setImmediate(async () => {
    try {
        const record = KernelOperationManifest.create({
            manifest_id:             'KOM-APEX-W2-ERA-1',
            era_ref:                 'APEX-CONSTITUTION-v1.0-ERA-1',
            established_at:          new Date().toISOString(),
            constitutional_basis:    'D4 §2.3; FoundingRatification; APEX-CONSTITUTION-v1.0; W2-02',
            class_b_operation_types: ['GATE_EVALUATION', 'SLOT_RESERVATION', 'SLOT_RELEASE', 'COMPENSATION_RECORD', 'INVARIANT_COMPILE', 'INVARIANT_EVALUATE', 'LATTICE_EVALUATE', 'FEEDBACK_RECORD', 'HEALTH_SIGNAL', 'REGISTRY_PRUNE', 'BOUND_MEMORY_WRITE'],
            manifest_version:        '1.0.0',
        });
        await constitutionalStore.write(record);
    } catch (err) {
        console.error('[constitutional-record] KernelOperationManifest init failed:', err?.message);
    }
});
