...
function _emit(record) {
    logger.debug('model-runtime', record.caller, { requestId: record.requestId, traceId: record.traceId || null, model: record.model, latency: record.latency, success: record.success });
    setImmediate(() => {
        try {
...
async function execute({ client, caller, model, system, messages, maxTokens = 2048, temperature, tier, purpose, traceId, taskId, tools }) {
    ...
    try {
        ...
        _emit({ requestId, traceId: resolvedTraceId, taskId, caller, source: caller,
        ...
        setImmediate(() => {
            try {
                ...
                gov.appendEvidenceBlock({
                    requestId, traceId: resolvedTraceId, taskId: taskId || null, caller,
                    model: resolvedModel, tier: resolvedTier,
                    latency: Date.now() - t0, inputTokens: 0, outputTokens: 0,
                    costEstimate: 0, success: false, failureType,
                    ts: new Date().toISOString(),
                }, 'llm_invocations').catch(() => {});
            } catch (_) {}
        });
        throw err;
    }
    ...
    // C3: ARCH-14 Phase 8 — record actual cost in resource_consumption (FAIL-SOFT)
    setImmediate(async () => {
        try {
            ...
            await sb.from('resource_consumption').insert({
    ...
    // C2: ARCH-14 Phase 7 / ARCH-11 §5.2 — persist MODEL_INVOKED to outbox before dispatch (FAIL-SOFT)
    setImmediate(async () => {
        try {
            const { createClient } = require('@supabase/supabase-js');
            if (!process.env.SUPABASE_URL) return;
            ...
            const payload = {
                requestId, traceId: resolvedTraceId, taskId: taskId || null, caller,
                model: resolvedModel, tier: resolvedTier,
                latency, inputTokens, outputTokens, costEstimate, success: true,
            };
            await sb.from('outbox').insert({
                idempotency_key: `MODEL_INVOKED:${requestId}`,
                source:          caller,
                type:            'MODEL_INVOKED',
                entity_refs:     [],
                payload,
    ...
    setImmediate(() => {
        try {
            ...
            outputCapture.capture(modelResult, taskId, resolvedTraceId).catch(() => {});
        } catch (_) {}
        try {
            const modelResult = {
                modelId:      resolvedModel,
...
function stream({ client, caller, model, system, messages, maxTokens = 4096, tier, traceId, taskId }) {
    ...
    return { requestId, stream: sdkStream, meta: { caller, model: resolvedModel, tier: resolvedTier, traceId: resolvedTraceId } };
}
...
function voice({ caller, model, traceId, taskId }) {
    ...
    _emit({ requestId, traceId: resolvedTraceId, taskId, caller, source: caller, provider,
    ...
    return { requestId, traceId: resolvedTraceId };
}
