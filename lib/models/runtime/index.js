...
// ── Circuit breaker — 5 consecutive failures → open, exponential cooldown ────
...
const _circuitBreaker = {
    ...
    cooldown() {
        ...
        return Math.min(60_000 * Math.pow(2, extra), 900_000);
    },
...
async function _callWithRetry(fn, maxRetries = 3) {
...
async function execute({ client, caller, model, system, messages, maxTokens = 2048, temperature, tier, purpose, traceId, taskId }) {
    ...
    try {
        result       = await _callWithRetry(() => callClient.messages.create(params));
        success      = true;
