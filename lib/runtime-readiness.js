...
async function _fetchEvidence() {
    ...
    const [
        ...
        sb.from('execution_snapshots').select('id', { count: 'exact', head: true }).not('task_id', 'like', 'PROBE%'),
        sb.from('execution_artifacts').select('id', { count: 'exact', head: true }).not('task_id', 'like', 'PROBE%'),
        sb.from('lesson_sources').select('id', { count: 'exact', head: true }).not('task_id', 'like', 'PROBE%'),
        ...
        sb.from('cost_accounting').select('tokens_in, tokens_out').not('task_id', 'like', 'PROBE%').limit(200),
        ...
        sb.from('cost_accounting').select('task_id, trace_id').not('task_id', 'like', 'PROBE%').order('created_at', { ascending: false }).limit(1),
    ]);
