...
async function runIntelligenceMinistry() {
  ...
  let opportunityCount = 0, knowledgeItems = 0, newsIngested = 0;

  // Bridge: apex_news_cache → civilization_events via global-intelligence-engine
  // Without this, opportunity-engine has no evidence base and produces 0 opportunities
  try {
    const since = new Date(Date.now() - 25 * 60 * 60 * 1000).toISOString(); // 25h window (runs every 6h, overlap for safety)
    const { data: articles } = await _sb()
    ...
    if (articles && articles.length) {
      const gig = require('../intelligence/global-intelligence-engine');
      // Group by category → domain mapping
      const domainMap = { technology: 'technology', business: 'economic', science: 'ai_industry', world: 'geopolitical', uk: 'geopolitical' };
      const byDomain = {};
      for (const a of articles) {
        const domain = domainMap[a.category] || 'news';
        if (!byDomain[domain]) byDomain[domain] = [];
        byDomain[domain].push({ title: a.title, body: a.summary || '', source: a.source || 'BBC News', url: a.url || '', timestamp: a.published_at });
      }
      for (const [domain, signals] of Object.entries(byDomain)) {
        try { await gig.ingest(domain, signals); newsIngested += signals.length; } catch {}
      }
    }
  } catch (e) {
    log.warn('ministry-intelligence', 'news→intelligence bridge error (non-fatal)', { error: e.message });
  }
  ...
  await _checkpoint('ministry:intelligence:last_run', { ts, opportunityCount, knowledgeItems, newsIngested, status: 'ok' });
  log.info('ministry-intelligence', 'cycle complete', { opportunityCount, knowledgeItems, newsIngested });
}
