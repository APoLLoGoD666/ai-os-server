...
// V-11-H-B-C: humanId is an optional trailing parameter (backward-compatible)
// that persists the parent-task owner into agent_actions.human_id. Legacy
// callers that omit it fall back to NULL — the H-B-C policy explicitly
// treats NULL as legitimate system-level provenance rather than false-Master.
...
async function pgCreateAgentTask(goal, status, plan = "", contextJson = null, actionsJson = null, createdBy = null, assignedAgentId = null) {
    const insert = { goal, status, plan: plan || '', context_json: contextJson, actions_json: actionsJson };
    if (createdBy)       insert.created_by        = createdBy;
    if (assignedAgentId) insert.assigned_agent_id = assignedAgentId;
    const data = check(
        await supabase.from('agent_tasks').insert(insert).select(),
        'pgCreateAgentTask'
    );
    return data?.[0] || null;
}
...
async function pgInsertToolExecution({ task_id, agent_id, tool_name, input, output, cost_usd, duration_ms }) {
    const data = check(
        await supabase.from('tool_executions')
            .insert({ task_id: task_id || null, agent_id: agent_id || null, tool_name, input: input || null, output: output || null, cost_usd: cost_usd || null, duration_ms: duration_ms || null })
            .select('id'),
...
async function pgInsertApproval({ task_id, tool_execution_id, approved_by, action_type, pattern, is_standing, expires_at }) {
    const data = check(
        await supabase.from('approvals')
            .insert({ task_id: task_id || null, tool_execution_id: tool_execution_id || null, approved_by, action_type, pattern: pattern || null, is_standing: !!is_standing, expires_at: expires_at || null })
            .select(),
        'pgInsertApproval'
    );
    return data?.[0] || null;
...
async function pgListApprovals({ is_standing, action_type, approved_by } = {}) {
    let q = supabase.from('approvals')
        .select()
        .is('revoked_at', null)
        .order('granted_at', { ascending: false })
        .limit(100);
    if (is_standing !== undefined) q = q.eq('is_standing', is_standing);
    if (action_type)  q = q.eq('action_type', action_type);
    if (approved_by)  q = q.eq('approved_by', approved_by);
    const data = check(await q, 'pgListApprovals');
    return data || [];
}

/* =========================
   KERNEL — AGENT TASKS (updated)
========================= */

async function pgCreateAgentTaskOwned(goal, status, plan, contextJson, actionsJson, createdBy) {
    const insert = { goal, status, plan: plan || '', context_json: contextJson, actions_json: actionsJson };
    if (createdBy) insert.created_by = createdBy;
    const data = check(
        await supabase.from('agent_tasks').insert(insert).select(),
        'pgCreateAgentTaskOwned'
    );
    return data?.[0] || null;
}
