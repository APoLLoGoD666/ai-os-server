...
// Voice chat history — all conversations stored via gateway layer 2 with source='voice_chat'.
// Returns parsed {user, assistant} pairs sorted newest first.
async function getVoiceHistory({ limit = 50, before = null, after = null } = {}) {
    try {
        let q = _sb().from('episodic_memory')
            .select('memory_id, objective, created_at, task_id')
        ...
        return (data || []).map(row => {
            let parsed = null;
            try { parsed = JSON.parse(row.objective); } catch (_) {}
            return {
        ...
        console.error(`[episodic-memory-pg] getVoiceHistory failed: ${e.message}`);
        return [];
    }
}
...
module.exports = { storeEpisode, findSimilar, getRecent, getFailures, getSuccessRate, getStats, getVoiceHistory };
