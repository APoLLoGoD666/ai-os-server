...
async function storeEpisode(episode, governance = {}) {
    ...
    const payload  = {
        ...
        source:          governance.source || 'orchestrator',
    ...
    setImmediate(async () => {
        try {
            ...
            if (embedding) {
                await _sb().from('episodic_memory')
                    .update({ embedding })
...
// Aggregate episode stats for reporting.
async function getStats() {
    try {
        const { data, error } = await _sb().from('episodic_memory')
            .select('success, cost_usd, duration_ms, complexity, failed_stage')
            .eq('status', 'validated')
            .order('created_at', { ascending: false })
            .limit(500);
        if (error) throw error;
        ...
        for (const e of failures) {
            if (e.failed_stage) stageFreq[e.failed_stage] = (stageFreq[e.failed_stage] || 0) + 1;
        }
        const topFailStage = Object.entries(stageFreq).sort((a,b) => b[1]-a[1])[0]?.[0] || null;
