...
async function storeFact(fact, category, options = {}) {
    ...
    setImmediate(async () => {
        try {
            ...
            if (embedding) {
                const { error: embErr } = await _sb().from('semantic_memory').update({ embedding }).eq('memory_id', memoryId);
                if (embErr) console.warn(`[semantic-memory] embed update failed: ${embErr.message}`);
            }
        } catch (e) {
...
async function search(query, options = {}) {
    ...
    // Semantic path — 5s timeout guards against cold embedding API stalls blocking the page
    try {
        const embedding = await Promise.race([
            embedText(query),
            new Promise((_, reject) => setTimeout(() => reject(new Error('embedText timeout')), 5000)),
        ]);
        if (embedding) {
    ...
    try {
        let q = _sb().from('semantic_memory')
            .select('memory_id, fact, category, domain, confidence, status, source, tags')
...
async function supersede(oldMemoryId, newFact, category, options = {}) {
    try {
        const { error: supErr1 } = await _sb().from('semantic_memory').update({
            status:        'superseded',
            superseded_by: 'pending',
            updated_at:    new Date().toISOString(),
        }).eq('memory_id', oldMemoryId);
        if (supErr1) throw supErr1;
        const newId = await storeFact(newFact, category, { ...options, confidence: 0.7 });
        if (newId) {
            const { error: supErr2 } = await _sb().from('semantic_memory').update({ superseded_by: newId })
                .eq('memory_id', oldMemoryId);
