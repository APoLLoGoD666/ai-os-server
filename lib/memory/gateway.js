...
async function getContext({ taskId, description, category, complexity,
                             ...
                             knowledgeRequirements = null }) {  // KG-04: optional sufficiency assessment
  ...
  if (hit) {
    ...
    // KG-04: knowledge sufficiency is never cached — always fresh on cache hit
    if (knowledgeRequirements && knowledgeRequirements.length > 0) {
      return _addKnowledgeSufficiency(hit, knowledgeRequirements, requestingEntity);
    }
    return hit;
  }
  ...
  cache.set(cacheKey, pkg, 60_000);  // cache WITHOUT knowledge_sufficiency — always freshly computed
  ...
  // KG-04: Add knowledge sufficiency as the final layer — never cached, always fresh
  if (knowledgeRequirements && knowledgeRequirements.length > 0) {
    return _addKnowledgeSufficiency(pkg, knowledgeRequirements, requestingEntity);
  }
  return pkg;
}

// KG-04: Compute knowledge sufficiency and return a new context package that includes it.
// Returns a new object (spread) so the cached pkg is never mutated.
async function _addKnowledgeSufficiency(basePkg, requirements, requestingEntity) {
  try {
    const kc = require('../knowledge/knowledge-context');
    const ks = await kc.buildKnowledgeContext(requirements, { assessed_by: requestingEntity });
    return { ...basePkg, knowledge_sufficiency: ks };
  } catch (_e) {
    return { ...basePkg, knowledge_sufficiency: null };
  }
}
...
async function storeMemory({ layer, content, tags = [], source, taskId, traceId,
  ...
  try { require('../viz-broadcaster').emit({ type: 'memory', layer, source: source || requestingEntity }); } catch (_) {}
  return result;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// getHistoricalState — constitutional historical query interface (W2-01)
// Returns HistoricalStateQueryResult (RT-07) for every authorized historical query.
// Callers may supply petlTxId to link the result to a PETL provenance chain.
// ─────────────────────────────────────────────────────────────────────────────
async function getHistoricalState({ query = '', entityRef = null, layers = [2, 7, 10],
                                     limit = 10, requestingEntity = 'agent',
                                     petlTxId = null } = {}) {
  // Access check — throws AccessDeniedError on unauthorized access
  ctrl.check(requestingEntity, layers, 'READ');

  const queryId        = `HSQR-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
  const queryTimestamp = new Date().toISOString();
  const searchQuery    = query || entityRef || '';

  let historicalLayers = [];
  let status           = 'VALID';

  try {
    const results = await searchMemory({ query: searchQuery, layers, limit, requestingEntity });
    historicalLayers = results;
    status = results.length > 0 ? 'VALID' : 'PARTIAL';
  } catch (err) {
    status = 'UNAVAILABLE';
    logger.warn('gateway', 'getHistoricalState query failed', { error: err.message, queryId });
  }

  const createData = {
    query_id:                queryId,
    query_timestamp:         queryTimestamp,
    historical_layers:       historicalLayers,
    temporal_validity_ms:    120_000,
    status,
    completeness_attestation: `layers=${layers.join(',')}; entity=${requestingEntity}; count=${historicalLayers.length}${petlTxId ? `; petl_tx=${petlTxId}` : ''}`,
  };
  if (petlTxId) {
    createData.provenance_segments = [{ petl_tx_id: petlTxId, requesting_entity: requestingEntity, timestamp: queryTimestamp }];
  }

  const queryResult = HistoricalStateQueryResult.create(createData);

  // Constitutional wiring — fire-and-forget (CONSTITUTIONAL WIRING PATTERN v1.0)
  const _qr = queryResult;
  setImmediate(async () => {
    try {
      await constitutionalStore.write(_qr);
    } catch (err) {
      console.error('[constitutional-record] HistoricalStateQueryResult failed:', err?.message);
    }
  });
