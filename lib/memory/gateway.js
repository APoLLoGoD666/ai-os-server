...
async function getContext({ taskId, description, category, complexity,
  ...
  const [founderCtx, lessons, policies, historical, projectCtx, semanticFacts] = await Promise.allSettled([
  ...
  const pkg = {
    ...
    assembly_metadata: {
      layers_queried: [0, 2, 3, 5, 7, 8, 9, 10, 11],
  ...
  if (layers.includes(8)) {
    try {
      const { data } = await _sb()
        ...
        .ilike('name', `%${query.slice(0, 60).replace(/%/g, '')}%`)
        .order('created_at', { ascending: false })
        .limit(limit);
      results.push(...(data || []).map(r => ({ layer: 8, id: r.id, content: r.name, entity_type: r.type, created_at: r.created_at })));
    } catch {}
  }
...
async function storeMemory({ layer, content, tags = [], source, taskId, traceId,
  ...
  switch (layer) {
    ...
    case 4:  throw new Error('Gateway: layer 4 (associative/contextual) is reserved — not yet implemented. Use layer 9 (semantic) for fact associations or layer 2 (episodic) for contextual records.');
