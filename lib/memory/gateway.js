...
async function getContext({ taskId, description, category, complexity,
  ...
  const [founderCtx, lessons, policies, historical, projectCtx, semanticFacts, workingMem, skillMem, kgNodes] = await Promise.allSettled([
  ...
  const pkg = {
    ...
    assembly_metadata: {
      layers_queried: [0, 1, 2, 3, 5, 6, 7, 8, 9, 10, 11],
...
async function storeMemory({ layer, content, tags = [], source, taskId, traceId,
  ...
  switch (layer) {
    ...
    case 4:  result = await mem.semanticMemory.storeFact(sanitized, 'association', { source, evidence: source, domain: 'associative' }); break;
