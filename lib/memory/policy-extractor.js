        ...
        try {
            ...
            try {
                ...
                await gateway.storeMemory({
                    layer:            10,
                    content:          policyText,
                    source:           `policy.extractor.${domain}`,
                    taskId:           `policy-extraction-${domain}`,
                    traceId:          `policy.extractor.${domain}`,
                    requestingEntity: 'policy-extractor',
                });
            } catch (storeErr) {
                console.error(`[policy-extractor] insert failed for ${domain}: ${storeErr.message}`);
                result.errors++;
                continue;
            }
