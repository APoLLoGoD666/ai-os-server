...
// taskId: task ID used to look up the corresponding decision_memory row
async function recordInfluence(lessonText, taskId, decisionType = 'operational') {
    try {
        // Look up the decision_memory row by task_id to get the canonical memory ID
        let decisionMemoryId = null;
        if (taskId) {
            const { data: dmRows } = await _sb().from('decision_memory')
                .select('id')
                .eq('task_id', taskId)
                .order('created_at', { ascending: false })
                .limit(1);
            if (dmRows && dmRows.length > 0) decisionMemoryId = dmRows[0].id;
        }
        ...
        await _sb().from('reflexion_records').update({
            ...
            validation_evidence:      { decisionMemoryId, taskId, decisionType, verifiedAt: new Date().toISOString() },
            updated_at:               new Date().toISOString(),
        }).eq('reflexion_id', record.reflexion_id);
