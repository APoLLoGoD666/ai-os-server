...
async function storeProcedure(name, procedureType, steps, options = {}) {
    ...
    setImmediate(async () => {
        try {
            ...
            if (embedding) {
                const { error: embErr } = await _sb().from('procedural_memory').update({ embedding }).eq('memory_id', memoryId);
                if (embErr) console.warn(`[procedural-memory] embed update failed: ${embErr.message}`);
            }
        } catch (e) {
...
async function recordExecution(memoryId, success, durationMs = null) {
    try {
        ...
        const { error: updErr } = await _sb().from('procedural_memory').update(update).eq('memory_id', memoryId);
