...
async function _processItem(item) {
    ...
    if (consolidation_stage === 'reflected') {
        ...
        if (memoryId) {
            ...
            // Auto-create knowledge graph node and edges
            setImmediate(async () => {
                try {
                    ...
                    const _syncResult = await knowledgeGraph.syncFromMemory(
                        nodeType, memoryId, classification.targetMemoryType, label, { source_type, source_id }
                    );
                    // syncFromMemory returns node_id string on cache-hit, object on new node
                    const _nodeId = typeof _syncResult === 'string' ? _syncResult : _syncResult?.node_id;
                    // Create GENERATED edges: link lesson/semantic nodes back to parent episode
                    if (_nodeId && source_id && ['lesson', 'semantic_memory'].includes(classification.targetMemoryType)) {
                        const { data: parents } = await _sb().from('knowledge_graph_nodes')
                            .select('node_id')
                            .eq('source_table', 'episodic_memory')
                            .filter('properties->>source_id', 'eq', String(source_id))
                            .limit(3);
                        for (const p of (parents || [])) {
                            await knowledgeGraph.createEdge(p.node_id, _nodeId, 'GENERATED', `consolidation:${source_id}`, 0.7).catch(() => {});
                        }
                    }
                } catch (e) {
...
async function _classify(content, sourceType) {
    ...
    try {
        ...
        const { result: resp } = await runtime.execute({
            client, caller: 'consolidation-engine',
