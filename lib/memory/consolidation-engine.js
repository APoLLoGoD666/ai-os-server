        ...
        if (memoryId) {
            ...
            setImmediate(async () => {
                try {
                    ...
                    const newNode  = await knowledgeGraph.syncFromMemory(
                    ...
                    if (newNode?.node_id && source_id && ['lesson', 'semantic_memory'].includes(classification.targetMemoryType)) {
                        ...
                        for (const p of (parents || [])) {
                            await knowledgeGraph.createEdge(p.node_id, newNode.node_id, 'GENERATED', `consolidation:${source_id}`, 0.7).catch(() => {});
                        }
                    }
                } catch (e) {
