...
async function storeDecision(decision, decisionType, options = {}) {
    ...
    setImmediate(async () => {
        try {
            ...
            if (embedding) {
                const { error: embErr } = await _sb().from('decision_memory').update({ embedding }).eq('memory_id', memoryId);
                if (embErr) console.warn(`[decision-memory] embed update failed: ${embErr.message}`);
            }
        } catch (e) {
