...
async function runWeeklyCycle() {
    ...
    try {
        ...
        // Promote behavior changes to cognitive_policy_settings.
        // Low/moderate risk → auto-activated immediately. High/critical → held for CIO review (active:false).
        // policy_value includes `.value` so cognitive-policy-engine can read it without structure mismatch.
        setImmediate(async () => {
            try {
                const now = new Date().toISOString();
                let activated = 0, pending = 0;
                for (const change of result.behavior_changes) {
                    const policyKey   = `adaptation:${(change.type || 'behavior')}:${(change.title || '').slice(0, 50).replace(/\s+/g, '_').toLowerCase()}`;
                    const highRisk    = change.estimatedImpact === 'high' || change.estimatedImpact === 'critical';
                    await _sb().from('cognitive_policy_settings').upsert({
                        policy_name:  policyKey,
                        policy_value: { value: change.description || change.title, title: change.title, description: change.description, estimatedImpact: change.estimatedImpact, cycleId },
                        applies_to:   'all',
                        active:       !highRisk,
                        applied_at:   now,
                    }, { onConflict: 'policy_name', ignoreDuplicates: false });
                    if (highRisk) pending++; else activated++;
                }
                if (result.behavior_changes.length > 0) {
                    console.log(`[adaptation-cycle] ${result.behavior_changes.length} behavior change(s) → ${activated} activated, ${pending} pending CIO review`);
                }
            } catch (e) {
                console.warn(`[adaptation-cycle] behavior→policy upsert failed: ${e.message}`);
            }
        });
