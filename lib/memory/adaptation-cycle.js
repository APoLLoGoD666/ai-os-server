...
async function runWeeklyCycle() {
    ...
    try {
        ...
        // Promote low-risk behavior changes to cognitive_policy_settings as pending proposals.
        // CIO entity reviews and activates; this closes the lesson→behavior application loop.
        setImmediate(async () => {
            try {
                ...
                for (const change of result.behavior_changes) {
                    ...
                    await _sb().from('cognitive_policy_settings').upsert({
                        ...
                        proposed_value: JSON.stringify({ title: change.title, description: change.description, estimatedImpact: change.estimatedImpact, cycleId }),
                        applies_to:     'all',
                        active:         false,
                        updated_at:     now,
                ...
                if (result.behavior_changes.length > 0) {
                    console.log(`[adaptation-cycle] ${result.behavior_changes.length} behavior change(s) proposed to cognitive_policy_settings`);
                }
            } catch (e) {
