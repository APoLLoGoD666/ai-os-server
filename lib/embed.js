...
async function _embedInternal(text, { dimensions = 768 } = {}) {
    ...
    if (voyageKey && Date.now() >= _voyage429Until) {
        try {
            const resp = await _post('api.voyageai.com', '/v1/embeddings',
                ...
                5000);
            if (resp?.data?.[0]?.embedding) return { embedding: resp.data[0].embedding, provider: 'voyage' };
        } catch (err) {
            if (err._status === 429) {
    ...
    if (googleKey) {
        ...
        try {
            const resp = await _post('generativelanguage.googleapis.com',
                ...
                8000);
            if (resp?.embedding?.values) return { embedding: resp.embedding.values, provider: 'gemini' };
        } catch (err) {
...
// Returns the embedding array only — backward-compatible for all existing callers.
async function embedText(text, opts) {
    const result = await _embedInternal(text, opts);
    return result ? result.embedding : null;
}

// Returns { embedding, provider } for callers that need provenance (e.g. vault_embeddings write path).
async function embedTextWithMeta(text, opts) {
    return _embedInternal(text, opts);
}

module.exports = { embedText, embedTextWithMeta };
