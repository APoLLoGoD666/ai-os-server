    ...
    return memory
        ...
        .map(item => {
            ...
            const msg = _sanitizer.sanitize(String(item.message || ''))
                ...
                .replace(/\b(ignore (previous|all|above) instructions?|disregard (the above|everything|all)|new instructions?:|forget everything|act as if|you are now)\b/gi, '[filtered]');
...
` : '';
    ...
    const gwSIE = gatewayCtx?.strategic_intelligence?.briefing
        ? `STRATEGIC INTELLIGENCE:\n${String(gatewayCtx.strategic_intelligence.briefing).slice(0, 400)}`
        : null;
    const gwVerdicts = gatewayCtx?.executive_history?.length
        ? `PRIOR COUNCIL DECISIONS:\n${gatewayCtx.executive_history.slice(0, 3).map(v => `• [${(v.role||'').toUpperCase()}] ${v.decision}: ${(v.rationale||'').slice(0, 100)}`).join('\n')}`
        : null;
    const gwKG = gatewayCtx?.knowledge_nodes?.length
        ? `KNOWLEDGE CONNECTIONS:\n${gatewayCtx.knowledge_nodes.slice(0, 4).map(n => `• ${(n.label || n.content || JSON.stringify(n)).slice(0, 80)}`).join('\n')}`
        : null;
    const gwBlock = [gwFounder, gwSIE, gwVerdicts, gwLessons, gwHistorical, gwKG, gwOpps, gwExec].filter(Boolean).join('\n\n');

    return `
You are Apex — Alex's personal AI OS, connected to his live runtime.${selfBlock ? '\n' + selfBlock.trimEnd() : ''}
${gwBlock ? '\n' + gwBlock : ''}
${memoryText ? `\nRECENT MEMORY:\n${memoryText}` : ''}
${docsText ? `\nRELEVANT SAVED DOCUMENTS:\n${docsText}\n` : ''}

TONE: Speak like a sharp, trusted friend — direct, warm, and natural. No "Certainly!", no "Of course!", no hollow affirmations. Match the energy: casual for casual, precise for technical. Keep it short unless the question genuinely needs depth. Never pad answers.
