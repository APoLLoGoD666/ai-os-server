...
// Uses 'orchestrator' (not 'certification') so Layer 0 access is granted and
// the production founder context — not the hardcoded fallback — is tested.
...
// Behavioral check: verify intelligence engine prompts reach provider context.
// Tests _adaptContext with task.description — proves WS1/Phase 23C fix is live.
function _behavioralIntelligenceDelivery() {
    try {
        const AnthropicModel = require('../models/providers/anthropic');
        const testModel      = new AnthropicModel('test', {});
        const MARKER         = 'INTEL_DELIVERY_PROOF_b3f9a12';
        const adapted        = testModel._adaptContext({ task: { description: MARKER } });
        const delivered      = (adapted.messages?.[0]?.content || '').includes(MARKER);
        return { delivered, ok: delivered };
    } catch (e) {
        return { delivered: false, ok: false, error: e.message };
    }
}

// Behavioral check: verify executive entities produce differentiated system prompts.
// Tests _adaptContext with executive_context — proves WS2/Phase 23C fix is live.
function _behavioralExecutiveDifferentiation() {
    try {
        const AnthropicModel = require('../models/providers/anthropic');
        const testModel      = new AnthropicModel('test', {});
        const cfoAdapted = testModel._adaptContext({
            task: { description: 'test' },
            executive_context: { system_prompt: 'You are APEX CFO.', executive_role: 'Chief Financial Officer' },
        });
        const ctoAdapted = testModel._adaptContext({
            task: { description: 'test' },
            executive_context: { system_prompt: 'You are APEX CTO.', executive_role: 'Chief Technology Officer' },
        });
        const differentiated = cfoAdapted.system !== ctoAdapted.system;
        const cfoHasRole     = (cfoAdapted.system || '').includes('Chief Financial Officer');
        const ctoHasRole     = (ctoAdapted.system || '').includes('Chief Technology Officer');
        return { differentiated, cfoHasRole, ctoHasRole, ok: differentiated && cfoHasRole && ctoHasRole };
    } catch (e) {
        return { differentiated: false, ok: false, error: e.message };
    }
}

// Behavioral check: verify policy table is queryable and returns DB rows (not fallback).
async function _behavioralPolicyRetrieval() {
    try {
        const gateway = require('../memory/gateway');
        const policies = await gateway.retrievePolicies({
            taskCategory: 'all',
            complexity:   'moderate',
            requestingEntity: 'orchestrator',
        });
        // Fallback _defaultPolicies only has retrieval_limit, planning_strategy, etc.
        // DB-sourced policies have named entries like 'default_plan_depth'.
        const fromDB = !!(policies?.cognitive?.default_plan_depth || policies?.cognitive?.default_autonomy_threshold);
        const policyCount = Object.keys(policies?.cognitive || {}).length;
        return { fromDB, policyCount, ok: fromDB };
    } catch (e) {
        return { fromDB: false, policyCount: 0, ok: false, error: e.message };
    }
}

// Phase 24: verify autonomous defence infrastructure is present and functional
function _checkDefenseMechanisms() {
    const results = {};
    try {
        const monitor = require('../health/monitor');
        const hs = monitor.getHealthState();
        results.monitor = { ok: !!hs && typeof hs.status === 'string', status: hs?.status };
    } catch (e) {
        results.monitor = { ok: false, error: e.message };
    }
    try {
        const { detect, classify } = require('../health/anomaly-detector');
        const _mon = require('../health/monitor');
        const fakeHealth = {
            status: 'critical',
            components: {
                ...
                policy:      { fromDB: true },
                certification: { lastResult: false },
            },
            thresholds: _mon.THRESHOLDS,
        };
        const anomalies = detect(fakeHealth);
        const summary   = classify(anomalies);
        const detectedCritical = anomalies.some(a => a.type === 'PROVIDER_UNAVAILABLE');
        const detectedCertFail = anomalies.some(a => a.type === 'CERTIFICATION_FAILED');
        results.anomalyDetector = { ok: detectedCritical && detectedCertFail, detectedCritical, detectedCertFail, classifiedAs: summary.status };
    } catch (e) {
        results.anomalyDetector = { ok: false, error: e.message };
    }
    try {
        const cont = require('../health/containment');
        results.containment = { ok: typeof cont.evaluateAndContain === 'function' && typeof cont.isContained === 'function' };
    } catch (e) {
        results.containment = { ok: false, error: e.message };
    }
    try {
        const sel = require('../models/selector');
        results.failover = { ok: typeof sel.withFailover === 'function' };
    } catch (e) {
        results.failover = { ok: false, error: e.message };
    }
    return results;
}
...
async function checkClause2(_inject = {}) {
    ...
    try {
        ...
        // Phase 23C/WS3: verify reflexion write failures are now observable (not silenced)
        const gatewaySrc = fs.existsSync(path.join(__dirname, '../../lib/memory/gateway.js'))
            ? fs.readFileSync(path.join(__dirname, '../../lib/memory/gateway.js'), 'utf8') : '';
        const reflexionSrc = fs.existsSync(path.join(__dirname, '../../lib/memory/reflexion-tracker.js'))
            ? fs.readFileSync(path.join(__dirname, '../../lib/memory/reflexion-tracker.js'), 'utf8') : '';
        const retrievalWriteLogged  = gatewaySrc.includes('recordRetrieval failed');
        const appliedStatusIncluded = reflexionSrc.includes("'pending','validated','applied'");
        result.evidence.push({
            check: 'Phase 23C: reflexion write failures surface via logger',
            value: retrievalWriteLogged ? 'LOGGED' : 'SILENCED',
            trust: 'A',
            note:  'A: reads gateway.js source; .catch(() => {}) swallows failures; logger.warn makes them visible',
        });
        result.evidence.push({
            check: 'Phase 23C: recordInfluence includes applied records',
            value: appliedStatusIncluded ? 'FIXED' : 'FROZEN',
            trust: 'A',
            note:  "A: reads reflexion-tracker.js; ['pending','validated'] filter froze proven lessons",
        });
        if (!retrievalWriteLogged) result.failures.push('reflexion write failures silenced — recordRetrieval errors invisible');
        if (!appliedStatusIncluded) result.failures.push("recordInfluence still excludes 'applied' records — proven lessons frozen");

        // Phase 24: verify autonomous defence infrastructure
        if (!_inject.skip_defense) {
            const defense = _checkDefenseMechanisms();
            result.evidence.push({
                check: 'Phase 24: health monitor operational',
                value: defense.monitor?.ok ? `OPERATIONAL (status=${defense.monitor.status})` : `FAILED: ${defense.monitor?.error || 'not functional'}`,
                trust: 'A',
                note:  'A: calls getHealthState() directly; verifies in-process health state machine is live',
            });
            result.evidence.push({
                check: 'Phase 24: anomaly detector classifies provider+cert failures',
                value: defense.anomalyDetector?.ok ? `CLASSIFIED (${defense.anomalyDetector.classifiedAs})` : `FAILED: ${defense.anomalyDetector?.error || 'detection broken'}`,
                trust: 'A',
                note:  'A: injects controlled fault state; verifies PROVIDER_UNAVAILABLE + CERTIFICATION_FAILED detected',
            });
            result.evidence.push({
                check: 'Phase 24: containment + failover modules present',
                value: (defense.containment?.ok && defense.failover?.ok) ? 'PRESENT (evaluateAndContain + withFailover)' : `MISSING: containment=${defense.containment?.ok}, failover=${defense.failover?.ok}`,
                trust: 'A',
                note:  'A: verifies function exports exist — structural proof defence architecture is wired in',
            });
            if (!defense.monitor?.ok)        result.failures.push('health monitor not operational — Phase 24 WS1 absent');
            if (!defense.anomalyDetector?.ok) result.failures.push('anomaly detector cannot classify faults — Phase 24 WS2 absent');
            if (!defense.containment?.ok)     result.failures.push('containment module missing — Phase 24 WS3 absent');
            if (!defense.failover?.ok)        result.failures.push('withFailover not exported from selector — Phase 24 WS3 failover absent');
        }
...
async function checkClause3(_inject = {}) {
    ...
    try {
        ...
        if (!_inject.skip_behavioral) {
            ...
            // Phase 23C/WS5: verify policy table is queryable and returns DB rows (not always fallback)
            if (!_inject.skip_policy_check) {
                const policyCheck = await _behavioralPolicyRetrieval();
                result.evidence.push({
                    check: 'Phase 23C: policy table schema correct and queryable',
                    value: policyCheck.ok
                        ? `DB rows returned (${policyCheck.policyCount} policies)`
                        : `fallback only — schema drift or empty table`,
                    trust: 'A',
                    note:  'A: executes retrievePolicies; DB hit means applies_to/active columns exist and migration ran',
                });
                if (!policyCheck.ok) result.failures.push('policy retrieval returns fallback only — schema drift or missing migration');
            }
        }
...
async function checkClause4(_inject = {}) {
    ...
    try {
        ...
        // Structural: Phase 16 + WS1 in server.js OR routes/voice-chat.js (extracted handler)
        const serverSrc    = fs.existsSync(path.join(__dirname, '../../server.js'))
            ? fs.readFileSync(path.join(__dirname, '../../server.js'), 'utf8') : '';
        const voiceChatSrc = fs.existsSync(path.join(__dirname, '../../routes/voice-chat.js'))
            ? fs.readFileSync(path.join(__dirname, '../../routes/voice-chat.js'), 'utf8') : '';
        const _combinedSrc   = serverSrc + voiceChatSrc;
        const phase16Present = _inject.phase16_present !== undefined ? _inject.phase16_present
            : _combinedSrc.includes('Phase 16');
        const ws1Present     = _inject.ws1_present    !== undefined ? _inject.ws1_present
            : _combinedSrc.includes('abstractForExternalPrompt');
        ...
        if (!_inject.skip_behavioral) {
            ...
            result.evidence.push({
                check: 'behavioral: gateway returns founder_context (via orchestrator, production path)',
                value: `present=${fcBehavior.present}, keys=${fcBehavior.keyCount}`,
                trust: 'A',
                note:  'A: orchestrator entity has Layer 0 access — tests production path, not hardcoded fallback',
            });
            if (!fcBehavior.ok) {
                result.failures.push('gateway.getContext() returned no founder_context keys — pipeline broken');
            }

            // Phase 23C/WS1: verify intelligence engine prompts reach provider context
            const intelDelivery = _inject.intel_delivery !== undefined ? _inject.intel_delivery
                : _behavioralIntelligenceDelivery();
            result.evidence.push({
                check: 'Phase 23C: intelligence engine prompt delivered to provider',
                value: intelDelivery.ok
                    ? 'DELIVERED — task.description present in adapted user message'
                    : `DROPPED — ${intelDelivery.error || 'task.description absent from provider payload'}`,
                trust: 'A',
                note:  'A: tests _adaptContext directly with marker string; proves WS1 fix: complete(p, { task: { description: p } })',
            });
            if (!intelDelivery.ok) result.failures.push('intelligence engine prompts not reaching provider — WS1 fix absent or broken');

            // Phase 23C/WS2: verify executive entities produce differentiated system prompts
            const execDiff = _inject.exec_differentiation !== undefined ? _inject.exec_differentiation
                : _behavioralExecutiveDifferentiation();
            result.evidence.push({
                check: 'Phase 23C: executive entities produce differentiated system prompts',
                value: execDiff.ok
                    ? `DIFFERENTIATED — CFO/CTO produce distinct system prompts with role context`
                    : `IDENTICAL — ${execDiff.error || 'executive_context stripped or not routed'}`,
                trust: 'A',
                note:  'A: tests _adaptContext with different executive_context; proves WS2 fix: roles survive abstraction',
            });
            if (!execDiff.ok) result.failures.push('executive entities produce identical prompts — executive_context not routed');
        }
...
// ── Clause 5: Constitutional Identity Preserved ───────────────────────────────
// Verifies the constitutional specification loads and core principles pass.
// Fails if constitutional protections disappear or principles degrade.

async function checkClause5(_inject = {}) {
    const result = {
        clause: 5,
        name:   'Constitutional identity preserved across all changes',
        pass:   false,
        evidence:  [],
        failures:  [],
        trust:     [],
    ...
    try {
        const constitSpec = require('../constitution/spec');

        // Structural: spec loads with required principles
        const count = constitSpec.PRINCIPLES.length;
        const cats  = constitSpec.CATEGORIES.length;
        result.evidence.push({
            check: 'constitutional spec loads',
            value: `${count} principles across ${cats} categories`,
            trust: 'A',
            note:  'A: requires lib/constitution/spec.js present and parseable',
        });
        if (_inject.spec_broken || count < 20) {
            result.failures.push(`constitutional spec has only ${count} principles — minimum 20 required`);
        }

        // Behavioral: all AUTHORITY + PRIVACY principles pass
        const authorityPrinciples = constitSpec.PRINCIPLES.filter(p => p.category === 'AUTHORITY' || p.category === 'PRIVACY');
        const verResults = _inject.verify_results || await Promise.all(
            authorityPrinciples.map(p => Promise.resolve(p.verify()).then(r => ({ id: p.id, name: p.name, ...r })))
        );
        const failed = verResults.filter(r => !r.pass);
        result.evidence.push({
            check: 'AUTHORITY + PRIVACY principles behavioral verification',
            value: `${verResults.length - failed.length}/${verResults.length} pass`,
            trust: 'A',
            note:  'A: executes actual behavioral checks; proves protections are operative, not just present in source',
        });
        for (const f of failed) {
            result.failures.push(`constitutional principle ${f.id} (${f.name}) failed: ${f.evidence}`);
        }

        // Behavioral: CERTIFICATION + HEALTH principles pass
        const critPrinciples = constitSpec.PRINCIPLES.filter(p => p.category === 'CERTIFICATION' || p.category === 'HEALTH');
        const critResults = _inject.crit_results || await Promise.all(
            critPrinciples.map(p => Promise.resolve(p.verify()).then(r => ({ id: p.id, name: p.name, ...r })))
        );
        const critFailed = critResults.filter(r => !r.pass);
        result.evidence.push({
            check: 'CERTIFICATION + HEALTH principles behavioral verification',
            value: `${critResults.length - critFailed.length}/${critResults.length} pass`,
            trust: 'A',
            note:  'A: proves certification gate and health defence are operative',
        });
        for (const f of critFailed) {
            result.failures.push(`constitutional principle ${f.id} (${f.name}) failed: ${f.evidence}`);
        }

        // Structural: drift-detector module present
        const ddSrc = fs.existsSync(path.join(__dirname, '../../lib/constitution/drift-detector.js'));
        result.evidence.push({
            check: 'drift-detector module present',
            value: ddSrc ? 'PRESENT' : 'MISSING',
            trust: 'A',
            note:  'A: structural — drift detection absent means no automatic drift protection',
        });
        if (!ddSrc) result.failures.push('drift-detector.js absent — constitutional drift would go undetected');

        // Structural: evolution-manager module present
        const emSrc = fs.existsSync(path.join(__dirname, '../../lib/constitution/evolution-manager.js'));
        result.evidence.push({
            check: 'evolution-manager module present',
            value: emSrc ? 'PRESENT' : 'MISSING',
            trust: 'A',
            note:  'A: structural — evolution pathway absent means constitutional changes have no governed route',
        });
        if (!emSrc) result.failures.push('evolution-manager.js absent — no authorized evolution pathway');
...
async function runFireDrill(clauseNumber) {
    const INJECTIONS = {
        ...
        5: { spec_broken: true, verify_results: [{ id: 'P01', name: 'test', pass: false, evidence: 'injected failure' }], crit_results: [] },
    };
    const inject = INJECTIONS[clauseNumber];
    if (!inject) throw new Error(`No fire-drill defined for clause ${clauseNumber}`);

    const fns = { 1: checkClause1, 2: checkClause2, 3: checkClause3, 4: checkClause4, 5: checkClause5 };
...
async function runAll() {
    ...
    const clauses = await Promise.all([
        checkClause1(), checkClause2(), checkClause3(), checkClause4(), checkClause5(),
    ]);
    const pass     = clauses.every(c => c.pass);
    const latency  = Date.now() - start;
    const failures = clauses.filter(c => !c.pass).flatMap(c => c.failures.map(f => `C${c.clause}: ${f}`));
    healthMonitor.recordCertificationResult(pass, failures);
    return {
        pass,
        clauses,
