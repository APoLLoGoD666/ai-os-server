...
async function _scanRoadmapGaps() {
    ...
    const { data } = await _sb().from('capability_registry').select('id').in('id', ids).catch(() => ({ data: [] }));
...
async function _scanFailurePatterns() {
    ...
    const { data } = await _sb().from('cron_run_log').select('job_name, status').gte('started_at', cutoff).catch(() => ({ data: [] }));
