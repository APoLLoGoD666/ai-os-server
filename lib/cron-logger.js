...
async function wrapCron(jobName, fn) {
    ...
    try {
        ...
        try { require('./viz-broadcaster').emit({ type: 'cron', job: jobName, status: 'ok', duration_ms: dur }); } catch (_) {}
    } catch (e) {
        const dur = Date.now() - started;
        ...
        try { require('./viz-broadcaster').emit({ type: 'cron', job: jobName, status: 'error', error: e.message }); } catch (_) {}
        throw e;
    } finally {
