...
async function assessRequirement(requirement_id, evidence = {}) {
    ...
    // Determine freshness if knowledge_type and formed_at provided
    let freshness_state = null;
    if (knowledge_type && formed_at) {
        ...
        freshness_state = 'UNKNOWN'; // formed_at provided but no TVW type to compare against
    }
    ...
    const { error } = await _sb().from('knowledge_evidence_assessments').insert({
        ...
        completeness:        completeness     != null ? completeness : null,
        has_contradictions,
        determination,
