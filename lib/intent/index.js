...
async function bridgeToReality(intentId, claimId, bridgeType = 'produced') {
    const { error } = await _sb().from('intent_reality_bridge').upsert(
        { intent_id: intentId, claim_id: claimId, bridge_type: bridgeType },
        { onConflict: 'intent_id,claim_id' }
    );
    if (error) throw new Error(`bridgeToReality failed: ${error.message}`);
}

module.exports = { INTENT_TYPES, recordIntent, closeAttribution, getOpenIntents, getIntentsByActor, intentAttributionRate, bridgeToReality };
