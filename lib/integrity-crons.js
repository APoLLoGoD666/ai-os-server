...
async function backup() {
    ...
    // Count rows in each audited table (IF EXISTS — new tables are added incrementally)
    const counts = {};
    for (const tbl of AUDIT_TABLES) {
        try {
            ...
            counts[tbl] = parseInt(rows[0].n, 10);
        } catch {
            counts[tbl] = null; // table not yet created
        }
    }
...
async function reconcile() {
    ...
    for (const src of SOURCES) {
        try {
            ...
            const ingested = parseInt(rows[0].n, 10);
            results.push({ source: src.name, ingested });
        } catch (e) {
...
function start() {
    ...
    // Backup: first run 10 min after startup, then every 24 hours
    setTimeout(() => wrapCron('integrity_backup', backup).catch(e =>
        _log.error('integrity', 'backup cron failed', { error: e.message })
    ), 10 * 60 * 1000);
    setInterval(() => wrapCron('integrity_backup', backup).catch(e =>
        _log.error('integrity', 'backup cron failed', { error: e.message })
    ), NIGHTLY_MS);

    // Reconciliation: first run 15 min after startup, then every 7 days
    setTimeout(() => wrapCron('integrity_reconcile', reconcile).catch(e =>
        _log.error('integrity', 'reconcile cron failed', { error: e.message })
    ), 15 * 60 * 1000);
    setInterval(() => wrapCron('integrity_reconcile', reconcile).catch(e =>
        _log.error('integrity', 'reconcile cron failed', { error: e.message })
    ), WEEKLY_MS);

    _log.info('integrity', 'integrity crons registered (backup: 24h, reconcile: 7d)');
}
