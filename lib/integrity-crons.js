...
const AUDIT_TABLES = [
    ...
    'entities', 'relationships', 'interactions', 'domain_scores', 'admission_rules',
];
...
function start() {
    ...
    const _domainScorer        = () => require('./civilization/domain-scorer').computeAndStore();
    const _admissionEngine     = () => require('./civilization/admission-engine').evaluateAll();
    const _consolidationEngine = () => require('./memory/consolidation-engine').process();

    const JOBS = [
        { name: 'integrity_backup',     fn: backup,                interval: NIGHTLY_MS      },
        { name: 'integrity_reconcile',  fn: reconcile,             interval: WEEKLY_MS       },
        { name: 'domain_scorer',        fn: _domainScorer,         interval: NIGHTLY_MS      },
        { name: 'admission_engine',     fn: _admissionEngine,      interval: WEEKLY_MS       },
        { name: 'consolidation_engine', fn: _consolidationEngine,  interval: 60 * 60 * 1000 },
    ];
    ...
    require('./memory/adaptation-cycle').repairStuckCycles().catch(() => {});
