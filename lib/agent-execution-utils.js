...
function normalizeExecutableAgentStep(step, originalRequest = "") {
    ...
    if (normalizedStep.type === "external_tool" && !normalizedStep.tool) {
        return {
            ok: false,
            reason: "Skipped invalid external_tool step (missing tool name)."
        };
    }
...
async function executeApprovedAgentActions(steps, options = {}) {
    ...
    for (let index = 0; index < steps.length; index += 1) {
        ...
        if (step.type === "external_tool") {
            const _registry = require('./tool-registry');
            let extResult;
            try {
                extResult = await _registry.execute(step.tool, step.params || {});
            } catch (err) {
                return { ok: false, message: `${step.tool} failed: ${err.message}`, results, undoEntries, skipped };
            }
            stepOutputs.push({ type: step.type, tool: step.tool, result: extResult });
            results.push(`${step.tool}: ${JSON.stringify(extResult).slice(0, 300)}`);
            continue;
        }
    }
