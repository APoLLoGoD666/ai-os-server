...
function shape(userMessage, rawReply, executionClass = 'EXECUTIVE', sessionId = null) {
    ...
    switch (mode) {
        case MODE.REFLEX:
            ...
            if (reply) bus.emit(bus.E.REFLEX_RESPONSE_SENT, { session_id: sessionId, text: reply });
            break;
...
// Lazy refs — avoids circular require at startup
let _pcmRef = null;
let _eaeRef = null;
let _speRef = null;

/**
 * Return unresolved cognitive threads for a session, if any.
 * Used to surface persistent-cognition context without altering orchestration logic.
 */
function getUnresolvedContext(sessionId) {
    if (!sessionId) return null;
    try {
        if (!_pcmRef) _pcmRef = require('./persistent-cognition-manager');
        const threads = _pcmRef.getThreadsForSession(sessionId);
        const unresolved = threads.filter(
            t => t.status === 'ACTIVE' || t.status === 'INTERRUPTED'
        );
        if (!unresolved.length) return null;
        return {
            has_unresolved: true,
            count:          unresolved.length,
            top_goal:       unresolved[0].goal,
            top_confidence: unresolved[0].confidence,
        };
    } catch (_) { return null; }
}

/**
 * Return the current executive focus context for a session.
 * Surfaces EAE active-focus hints for orchestration-layer use.
 */
function getExecutiveContext(sessionId) {
    if (!sessionId) return null;
    try {
        if (!_eaeRef) _eaeRef = require('./executive-arbitration-engine');
        const snap = _eaeRef.generateExecutiveSnapshot(sessionId);
        if (!snap.current_focus) return null;
        return {
            has_focus:         true,
            focus_goal:        snap.current_focus.goal,
            is_strategic:      snap.current_focus.is_strategic,
            priority:          snap.current_focus.priority,
            deferred_count:    snap.deferred_goal_count,
...
/**
 * Return active strategic objective context for a session.
 * Used to surface long-horizon continuity hints in orchestration.
 */
function getStrategicContext(sessionId) {
    if (!sessionId) return null;
    try {
        if (!_speRef) _speRef = require('./strategic-planning-engine');
        return _speRef.getStrategicContext(sessionId);
    } catch (_) { return null; }
}

module.exports = { shape, classifyIntent, determineMode, sessionState, counters, getUnresolvedContext, getExecutiveContext, getStrategicContext, INTENT, MODE };
