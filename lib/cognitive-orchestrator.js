...
const _sessions = new Map(); // sessionId → { intent, mode, pendingTools, claudeActive, lastActivity }

function _getSession(id) {
    if (!_sessions.has(id)) {
        _sessions.set(id, { intent: null, mode: null, pendingTools: 0, claudeActive: false, lastActivity: Date.now() });
    }
...
// ── Event bus subscriptions (read-only, non-blocking) ─────────────────────────

bus.on(bus.E.CLAUDE_STARTED, ({ payload }) => {
    if (!payload.session_id) return;
    const s = _getSession(payload.session_id);
    s.claudeActive = true; s.lastActivity = Date.now();
});

bus.on(bus.E.CLAUDE_FIRST_TOKEN, ({ payload }) => {
    if (!payload.session_id) return;
    _getSession(payload.session_id).lastActivity = Date.now();
});

bus.on(bus.E.TOOL_DISPATCHED, ({ payload }) => {
    if (!payload.session_id) return;
    const s = _getSession(payload.session_id);
    s.pendingTools++; s.lastActivity = Date.now();
});

bus.on(bus.E.TOOL_COMPLETED, ({ payload }) => {
    if (!payload.session_id) return;
    const s = _getSession(payload.session_id);
    s.pendingTools = Math.max(0, s.pendingTools - 1); s.lastActivity = Date.now();
});

bus.on(bus.E.USER_INTERRUPTED, ({ payload }) => {
    if (!payload.session_id) return;
    const s = _getSession(payload.session_id);
    s.pendingTools = 0; s.claudeActive = false;
});

bus.on(bus.E.SESSION_COMPLETED, ({ payload }) => {
    if (payload.session_id) _sessions.delete(payload.session_id);
});
...
function shape(userMessage, rawReply, executionClass = 'EXECUTIVE', sessionId = null) {
    ...
    const mode   = determineMode(intent, executionClass);

    // Update session state
    if (sessionId) {
        const s = _getSession(sessionId);
        s.intent = intent; s.mode = mode; s.lastActivity = Date.now();
    }
...
/**
 * Get current cognitive state for a session (for observability).
 */
function sessionState(sessionId) {
    return sessionId ? (_sessions.get(sessionId) || null) : null;
}
