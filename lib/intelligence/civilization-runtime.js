...
async function _tick() {
  ...
  if (_cycleCost >= CYCLE_BUDGET_USD) {
    ...
    result.phases.deliberate = await _phase('deliberate', cycleId, async () => {
      ...
      // Cooldown: skip if a civ-runtime deliberation already happened in the last 24h
      try {
        const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
        const { data: recent } = await require('../clients').getSupabaseClient()
          .from('executive_deliberations')
          .select('id')
          .eq('context->>source', 'civilization_runtime')
          .gte('created_at', since)
          .limit(1);
        if (recent && recent.length > 0) {
          return { deliberation: null, reason: 'cooldown: civ-runtime deliberated within 24h' };
        }
      } catch {}
