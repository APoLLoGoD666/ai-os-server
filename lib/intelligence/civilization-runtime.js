...
async function _tick() {
  ...
  result.phases.observe = await _phase('observe', cycleId, async () => {
    ...
    const scanResult = await gig.ingestRaw({
      type: 'civilization_tick',
      source: 'civilization-runtime',
      payload: {
    ...
    const normalizedScan = {
      eventsDetected: scanResult.stored ? 1 : 0,
    ...
    return { eventsDetected: normalizedScan.eventsDetected, graph_threats: graphThreats };
  });
  ...
  // PHASE 4: PLAN — generate or refresh strategy if needed
  result.phases.plan = await _phase('plan', cycleId, async () => {
    ...
    // Regenerate 90-day plan if none exists or it's > 30 days old
    const needs90Day = !existingPlans.find(p => {
      if (p.horizon !== '90_day') return false;
      const age = Date.now() - new Date(p.created_at).getTime();
      return age < 30 * 86_400_000;
    });

    if (needs90Day) {
      const plan = await strategyEngine.generate90Day();
      return { planGenerated: '90_day', planId: plan.id };
    }
    return { planGenerated: null, reason: 'plans up to date' };
  });
  ...
  result.phases.learn = await _phase('learn', cycleId, async () => {
    ...
    await gateway.storeMemory({
      ...
      tags:    ['civilization_cycle', `health_${result.phases.analyze?.output?.health?.classification || 'unknown'}`],
    ...
    return { lessonPersisted: true, durationMs: cycleMs };
  });

  // PHASE 7: UPDATE MEMORY — store strategic summary at layer 5
