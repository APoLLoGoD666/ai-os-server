...
async function _tick() {
  ...
  result.phases.observe = await _phase('observe', cycleId, async () => {
    ...
    result.phases.observe.genome_ok     = genomeOk;
    result.phases.observe.clock_domains = clockDomains;
  ...
  try {
    require('../clients').getSupabaseClient()?.from('civilization_cycle_log').upsert({
      cycle_id:       cycleId,
