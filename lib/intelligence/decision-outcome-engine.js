...
// Close a pending council decision outcome by deliberationId — called when the dispatch task completes
async function measureByDecisionId(decisionId, actualOutcome, variance, lessonsLearned) {
  try {
    const { data } = await _sb()
      .from('decision_outcomes')
      .select('id')
      .eq('decision_id', String(decisionId))
      .eq('status', 'pending')
      .maybeSingle();
    if (!data) return null;
    return measure({ id: data.id, actualOutcome, variance: variance || null, lessonsLearned: lessonsLearned || null });
  } catch { return null; }
}

module.exports = { record, measure, getPending, getMeasured, getSummary, recordCouncilDecision, measureByDecisionId };
