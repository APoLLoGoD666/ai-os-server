...
// F3: Rate-limit auto-deploy — max 1 per 24h, only allowlisted types
let _lastAutoDeployAt = 0;
const AUTO_DEPLOY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const AUTO_DEPLOY_ALLOWLIST   = new Set(['config_tweak', 'prompt_refinement', 'threshold_adjustment', 'cache_tuning']);
...
async function _autoDeploy(candidateId, riskLevel) {
    try {
        // F3: enforce rate-limit and type allowlist before auto-deploying
        const now = Date.now();
        if ((now - _lastAutoDeployAt) < AUTO_DEPLOY_INTERVAL_MS) {
            console.log(`[improvement-governor] auto-deploy rate-limited (last deploy < 24h ago): ${candidateId}`);
            return;
        }
        const { data: cand } = await _sb().from('improvement_candidates').select('improvement_type').eq('candidate_id', candidateId).single().catch(() => ({ data: null }));
        if (cand && !AUTO_DEPLOY_ALLOWLIST.has(cand.improvement_type)) {
            console.log(`[improvement-governor] auto-deploy blocked (type not allowlisted: ${cand.improvement_type}): ${candidateId}`);
            await _flagForReview(candidateId, `auto-deploy blocked: type "${cand.improvement_type}" not in allowlist — human review required`);
            return;
        }
