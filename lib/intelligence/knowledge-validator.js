...
async function submitLesson(lessonText, options = {}) {
    const { lessonSourceId, traceId, taskId, sourceType = 'lesson', obsRecordId, domainId } = options;
...
async function _promoteToKnowledge(item, confidence, contradictions) {
    ...
    // Wired exclusively at this genuine EP-T4 validation site.
    ...
    if (item.obs_record_id) {
        try {
            ...
            if (knowledgeId) {
                ...
                const dumId = await dumRegistry.formDomainUnderstanding({ knowledgeId, obsRecordId: item.obs_record_id, domainId: item.domain_id || null, item, confidence });
                ...
                if (dumId) {
                    ...
                    await cumRegistry.formCivilizationUnderstanding({ dumId, knowledgeId, obsRecordId: item.obs_record_id, item, confidence });
                }
            }
        } catch (_) {}
    }
