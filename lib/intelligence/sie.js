...
async function analyzeThreats() {
  ...
  // Security/privacy threats: unacknowledged anti-goal alerts — deduplicated by anti_goal pattern.
  // Many alerts may fire for the same underlying anti-goal; collapse them so the threat list
  // shows distinct risks, not one entry per trigger event.
  const _alertByGoal = new Map();
  for (const alert of intel.alerts) {
    const key = (alert.anti_goal || '').trim().toLowerCase().slice(0, 80) || alert.id;
    if (!_alertByGoal.has(key)) _alertByGoal.set(key, { alert, count: 1 });
    else _alertByGoal.get(key).count++;
  }
  for (const { alert, count } of Array.from(_alertByGoal.values()).slice(0, 5)) {
    const isCritical = /data.?leak|privacy|security|breach/i.test(alert.trigger_text || alert.anti_goal || '');
    const label = alert.anti_goal || alert.trigger_text || 'unknown';
    threats.push({
      id:          `alert_${alert.id}`,
      type:        isCritical ? 'security' : 'operational',
      severity:    alert.severity || 'high',
      description: `${label.slice(0, 120)}${count > 1 ? ` (triggered ${count}×)` : ''}`,
