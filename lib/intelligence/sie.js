...
async function analyzeThreats() {
  ...
  // Anti-goal alerts grouped by threat CATEGORY — each alert has unique anti_goal text so
  // deduping by text yields no collapsing. Instead bucket by keyword pattern → at most 5 categories.
  const _ALERT_CATS = {
    security:    /data.?leak|privacy|security|breach|expos|user.?data|credential|password/i,
    code_safety: /code|edit|modif|inject|execut|script|lib\/|write|overwrite|deploy/i,
    financial:   /financ|money|cost|payment|bank|wallet/i,
    health:      /health|mental|burnout|sleep|stress/i,
  };
  const _CAT_LABELS = {
    security:    'Data security & privacy violations',
    code_safety: 'Unsafe code modification requests',
    financial:   'Financial boundary violations',
    health:      'Health & wellbeing boundary violations',
    operational: 'Operational anti-goal violations',
  };
  const _alertByCat = new Map();
  for (const alert of intel.alerts) {
    const text = (alert.anti_goal || '') + ' ' + (alert.trigger_text || '');
    let cat = 'operational';
    for (const [name, rx] of Object.entries(_ALERT_CATS)) {
      if (rx.test(text)) { cat = name; break; }
    }
    if (!_alertByCat.has(cat)) _alertByCat.set(cat, { sample: alert, count: 1 });
    else _alertByCat.get(cat).count++;
  }
  for (const [cat, { sample: alert, count }] of _alertByCat) {
    threats.push({
      id:          `alert_cat_${cat}`,
      type:        cat === 'security' ? 'security' : cat === 'financial' ? 'financial' : 'operational',
      severity:    alert.severity || 'high',
      description: count === 1
        ? `${_CAT_LABELS[cat]}: ${(alert.anti_goal || alert.trigger_text || '').slice(0, 100)}`
        : `${_CAT_LABELS[cat]} (${count} violations detected)`,
...
Return JSON:
{
  ...
  let _traceRaw     = '(no AI call — deterministic fallbacks used)';
  let _traceParsed  = false;

  try {
    const { result } = await runtime.execute({ tier: 'balanced', caller: 'sie.briefing', messages: [{ role: 'user', content: prompt }], maxTokens: 700 });
    _traceRaw        = result.content[0]?.text || '';
    const match      = _traceRaw.match(/\{[\s\S]*\}/);
    if (match) {
      const parsed = JSON.parse(match[0]);
      if (parsed.biggest_opportunity) { briefing = { ...briefing, ...parsed }; _traceParsed = true; }
    }
  } catch {}
  ...
  // Evidence: the specific data items that drove each briefing field — surfaced in UI drill-down
  briefing._evidence = {
    biggest_opportunity: biggestOpp ? {
      type: 'opportunity',
      title: biggestOpp.title,
      description: biggestOpp.description,
      category: biggestOpp.category,
      score: biggestOpp.scores.opportunity_score,
      scores: {
        founder_alignment: biggestOpp.scores.founder_alignment,
        expected_value:    biggestOpp.scores.expected_value,
        empire_impact:     biggestOpp.scores.empire_impact,
        risk:              biggestOpp.scores.risk_score,
      },
      urgency: biggestOpp.urgency,
      all_count: scored_opps.length,
    } : null,
    biggest_threat: biggestThreat ? {
      type: 'threat',
      threat_type: biggestThreat.type,
      severity:    biggestThreat.severity,
      description: biggestThreat.description,
      score:       biggestThreat.score,
      source:      biggestThreat.source,
      all_count:   thrs.length,
      top_others:  thrs.slice(1, 3).map(t => ({ description: t.description, severity: t.severity, source: t.source })),
    } : null,
    biggest_bottleneck: biggestBn ? {
      type: 'bottleneck',
      bottleneck_type: biggestBn.type,
      label:           biggestBn.label,
      description:     biggestBn.description,
      impact_score:    biggestBn.impact_score,
      action:          biggestBn.action,
      blocked_goals:   biggestBn.blocked_goals,
      all_count:       bns.length,
    } : null,
    highest_leverage_action: highestLeverageGoal ? {
      type:                   'goal',
      label:                  highestLeverageGoal.label,
      progress_pct:           highestLeverageGoal.progress_pct,
      weight:                 highestLeverageGoal.weight,
      probability_of_success: highestLeverageGoal.probability_of_success,
      missing_requirements:   highestLeverageGoal.missing_requirements,
      horizon:                highestLeverageGoal.horizon,
    } : null,
    strategic_focus_this_week: {
      type: 'synthesis',
      model_inputs: `${scored_opps.length} opportunities · ${thrs.length} threats · ${bns.length} bottlenecks · ${gs.length} goals`,
      top_inputs: [
        biggestOpp    && `Opportunity: ${biggestOpp.title} (score ${biggestOpp.scores.opportunity_score})`,
        biggestThreat && `Threat: ${biggestThreat.description?.slice(0, 120)}`,
        biggestBn     && `Bottleneck: ${biggestBn.label} — ${biggestBn.description?.slice(0, 100)}`,
      ].filter(Boolean),
    },
    strategic_focus_this_month: {
      type: 'synthesis',
      model_inputs: `${gs.length} active goals · founder graph · empire context`,
      top_inputs: gs.slice(0, 4).map(g => `Goal: ${g.label} (${g.progress_pct || 0}% · ${g.horizon || 'ongoing'})`),
    },
  };
  if (perfStats) {
    const withData = Object.values(perfStats).filter(s => s.withOutcome > 0);
  ...
  // Full reasoning trace — stored separately so the briefing cache stays lean
  cache.set('sie:briefing:trace:v1', {
    generated_at: briefing.generated_at,
    opportunities_analyzed: scored_opps.map(o => ({
      id: o.id, title: o.title, category: o.category || null,
      description: (o.description || '').slice(0, 180), urgency: o.urgency,
      score: o.scores.opportunity_score, scores: o.scores,
    })),
    threats_analyzed: thrs.map(t => ({
      id: t.id, type: t.type, severity: t.severity,
      description: t.description, score: t.score, source: t.source,
    })),
    bottlenecks_analyzed: bns.map(b => ({
      type: b.type, label: b.label, description: b.description,
      impact_score: b.impact_score, action: b.action, blocked_goals: b.blocked_goals,
    })),
    goals_analyzed: gs.map(g => ({
      id: g.id, label: g.label, progress_pct: g.progress_pct, weight: g.weight,
      probability_of_success: g.probability_of_success, horizon: g.horizon,
      missing_requirements: g.missing_requirements,
    })),
    context: {
      graph_summary:  gCtx   ? String(gCtx.graph_summary   || '').slice(0, 500) : null,
      empire_summary: empireCtx ? String(empireCtx.empire_summary || '').slice(0, 500) : null,
      perf_summary:   perfSummary || null,
    },
    synthesis: { prompt, raw_response: _traceRaw, parse_success: _traceParsed },
  }, BRIEFING_CACHE_TTL);
  ...
  // Auto-trigger council on critical findings — fire-and-forget, never blocks briefing return
  setImmediate(async () => {
    try {
      const criticalThreat = thrs.find(t => t.severity === 'existential' || t.severity === 'critical');
      const topOpp = scored_opps[0];
      if (!criticalThreat && (!topOpp || topOpp.scores.opportunity_score < 75)) return;
      // Dedupe: skip if we already triggered council for this exact item in the current cache window
      const triggerKey = criticalThreat ? `t_${criticalThreat.id}` : `o_${topOpp.id}`;
      if (cache.get('sie:last_council_trigger') === triggerKey) return;
      cache.set('sie:last_council_trigger', triggerKey, BRIEFING_CACHE_TTL);
      const council = require('../executive/executive-council');
      const question = criticalThreat
        ? `SIE briefing detected a ${criticalThreat.severity} threat: ${(criticalThreat.description || '').slice(0, 200)}. Category: ${criticalThreat.category || 'operational'}. What immediate action should APEX take?`
        : `SIE briefing identified top strategic opportunity: "${topOpp.title}". Score ${topOpp.scores.opportunity_score}. Category: ${topOpp.category || 'business'}. Should APEX act on this now?`;
      await council.deliberate(question, { source: 'sie_auto', briefingAt: briefing.generated_at });
    } catch {}
  });
