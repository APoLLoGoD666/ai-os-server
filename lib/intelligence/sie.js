  ...
  setImmediate(async () => {
    try {
      ...
      const question = criticalThreat
        ? `SIE briefing detected a ${criticalThreat.severity} threat: ${(criticalThreat.description || '').slice(0, 200)}. What immediate action should APEX take?`
        : `SIE briefing identified top strategic opportunity: "${topOpp.title}". Score ${topOpp.scores.opportunity_score}. Should APEX act on this now?`;
