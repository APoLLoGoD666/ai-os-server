...
[{ "rank": number, "action": string (specific, ≤120 chars), "why": string (1 sentence, why this > alternatives), "impact": "high"|"medium"|"low", "type": "opportunity"|"bottleneck"|"threat"|"goal" }]`;
  ...
  try {
    const result = await model.complete(prompt, {}, { maxTokens: 800 });
...
Return JSON:
{
  ...
  try {
    const result = await model.complete(prompt, {}, { maxTokens: 700 });
    const match  = result.content.match(/\{[\s\S]*\}/);
    if (match) {
      const parsed = JSON.parse(match[0]);
...
Provide concise analysis (JSON):
{
  ...
  try {
    const result = await model.complete(prompt, {}, { maxTokens: 600 });
    const match  = result.content.match(/\{[\s\S]*\}/);
    if (match) {
      const parsed = JSON.parse(match[0]);
