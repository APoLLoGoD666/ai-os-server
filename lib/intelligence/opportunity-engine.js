...
Only include opportunities directly supported by cited evidence.`;
  ...
  let rawOpps = [];
  try {
    const { result } = await runtime.execute({ tier: 'balanced', caller: 'opportunity-engine', messages: [{ role: 'user', content: prompt }], maxTokens: 2000 });
  ...
  // V-11-J canonical evidence_refs contract: Array<{ label, source, ts }>.
  // Projects raw model refs ("EVT-0" | "MEM-2" | "SIG-1") into structured
  // objects the API returns directly and the frontend renders as-is.
  const nowIso = new Date().toISOString();
  function _projectRef(r) {
    if (r && typeof r === 'object' && (r.label || r.source)) {
      // Already structured — normalise fields defensively.
      return {
    ...
    if (ref.startsWith('EVT-')) {
      const e = events[parseInt(ref.replace('EVT-', ''))];
      if (e) return { label: e.title || `Event ${e.id}`, source: 'event', ts: e.occurred_at || e.created_at || nowIso };
      return { label: ref, source: 'event', ts: null };
    }
    if (ref.startsWith('MEM-')) {
      const m = memPatterns[parseInt(ref.replace('MEM-', ''))];
      if (m) {
        const snippet = typeof m.content === 'string' ? m.content.slice(0, 80) : (m.title || 'Memory');
        return { label: snippet, source: 'memory', ts: m.created_at || m.recorded_at || null };
      }
      return { label: ref, source: 'memory', ts: null };
    }
    if (ref.startsWith('SIG-')) {
      const s = marketSignals[parseInt(ref.replace('SIG-', ''))];
      return { label: s ? String(s).slice(0, 80) : ref, source: 'market_signal', ts: nowIso };
    }
    return { label: ref || 'Source', source: 'unknown', ts: null };
  }

  const rows = scored.map(opp => {
    const rawRefs = Array.isArray(opp.evidence_refs) ? opp.evidence_refs : [];

    const originEventIds = rawRefs
      .filter(r => typeof r === 'string' && r.startsWith('EVT-'))
      .map(r => eventIdMap[r])
      .filter(Boolean);

    const citedSignals = rawRefs
      .filter(r => typeof r === 'string' && r.startsWith('SIG-'))
      .map(r => signalMap[r])
      .filter(Boolean);

    const citedMemory = rawRefs
      .filter(r => typeof r === 'string' && r.startsWith('MEM-'))
      .map(r => {
    ...
    // V-11-J: canonical structured evidence_refs.
    const structuredRefs = rawRefs.map(_projectRef);
    ...
    return {
      ...
      // V-11-J: top-level canonical column (Array<{label,source,ts}>).
      evidence_refs:     structuredRefs,
      roi_forecast: {
        urgency:             opp.urgency || 'this_month',
        recommended_action:  opp.recommended_action || '',
        // Legacy nested location preserved for backwards compatibility.
        evidence_refs:       structuredRefs,
