...
Only include opportunities directly supported by cited evidence.`;
  ...
  // Filter: only keep opportunities with at least one evidence reference
  const evidenced = rawOpps.filter(o => Array.isArray(o.evidence_refs) && o.evidence_refs.length > 0);
  if (!evidenced.length) {
    logger.warn('opportunity-engine', 'no evidence-backed opportunities produced');
    return [];
  }
  ...
  // Get executive scores for top 3
