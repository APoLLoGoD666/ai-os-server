...
async function _scoreExecution() {
  ...
  try {
    ...
    const { data: runs } = await sb
      ...
      .select('success, duration_ms, retry_count, complexity')
    ...
    if (runs?.length) {
      ...
      // Penalise high retry rate: each avg retry above 1 costs 10 points
      const avgRetries  = runs.reduce((s, r) => s + (r.retry_count || 0), 0) / (runs.length || 1);
      const retryPenalty = Math.round(Math.max(0, avgRetries - 1) * 10);
      const baseScore   = successRate !== null ? Math.round(successRate * 100) : 60;
      const score       = Math.max(0, baseScore - retryPenalty);
      ...
      return { score, details: { runs_7d: runs.length, executed, completed, failed, success_rate: successRate, avg_retries: +avgRetries.toFixed(2) }, alerts };
    }
