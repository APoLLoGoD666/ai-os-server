...
// V-11-H-B1: optional owner filter. Pass { humanId, bypass } to scope
// results to a specific human_id. When bypass=true (Master) no filter applied.
async function _parseTasks(ownerScope = null) {
    try {
        let query = sbAdmin.from('apex_tasks').select('*').order('created_at');
        if (ownerScope && !ownerScope.bypass && ownerScope.humanId) {
            query = query.or(`human_id.eq.${ownerScope.humanId},human_id.is.null`);
        }
        ...
        return {
            ...
            awaitingApproval: tasks.filter(t => ['awaiting_approval','pending_approval','approval_required'].includes(t.status)),
...
// V-11-H-B-C: humanId parameter threads owner from parent task into notification
// row so downstream reads (GET /notifications scoped by human_id) surface it to
// the correct user. NULL is legitimate ONLY for system-initiated notifications
// with no originating human context (cron, capability monitor, etc.).
async function _appendNotif(message, type = 'info', humanId = null) {
    try {
...
// V-11-H-B-C: humanId parameter threads owner from parent task into timeline
// row so pipeline outcomes are visible to the originating user under scoped
// queries. Defaults to NULL only when no human context is available (system
// scheduled tasks).
async function _appendTimeline(entry, humanId = null) {
    try {
...
async function _startAutoPipeline(taskId) {
    ...
    const task = taskRow;
    // V-11-H-B-C: extract parent-task owner and thread it through every
    // downstream write (notif, timeline, agent-runs audit) so no
    // pipeline-generated row lands with human_id = NULL when the task is
    // User-owned. Master-owned tasks correctly carry Master's humanId.
    const humanId = task.human_id || null;
    ...
    const _markFailed = async (reason) => {
        try {
            await sbAdmin.from('apex_tasks')
                ...
                .eq('id', taskId);
            await _appendNotif(`❌ ${taskId} failed: ${reason}`, 'error', humanId);
        } catch {}
    };
    ...
    try {
        ...
        // V-11-H-B-C: emit humanId on lifecycle events so downstream listeners
        // (services/init.js AGENT_COMPLETED → apex_agent_runs insert) can
        // stamp the correct owner without a second Supabase lookup.
        _bus.emit(_bus.E.AGENT_STARTED, { task_id: taskId, label: spec.objective, human_id: humanId });
        const result = await runAgentTeam(spec, taskId);
        const duration = Date.now() - t0;
        _bus.emit(_bus.E.AGENT_COMPLETED, { task_id: taskId, elapsed_ms: duration, ok: result.success, human_id: humanId });

        if (result.success) {
            await sbAdmin.from('apex_tasks')
                .update({ status: 'completed', updated_at: new Date().toISOString() })
                .eq('id', taskId);
            await _appendNotif(`✅ ${taskId} completed — ${spec.objective}. Commit: ${result.commitHash}`, 'success', humanId);
            ...
            try {
                ...
                await obsidianMemory.writeProvenance(taskId, {
                    title:      spec.objective,
                    notesRead:  (spec.filesToModify || []).filter(f => /\.md$/i.test(f)),
                    toolsUsed:  spec.filesToModify || [],
                    outcome:    'completed',
                    commitHash: result.commitHash || '',
                });
            } catch (e) {
                console.warn('[AutoPipeline] provenance write failed:', e.message);
            }
        } else {
...
async function _runTask(taskId, res) {
    ...
    const task = taskRow;
    // V-11-H-B-C: extract parent-task owner so every notification generated by
    // this synchronous run carries the correct human_id (never NULL for
    // User-owned tasks).
    const humanId = task.human_id || null;
    await sbAdmin.from('apex_tasks')
    ...
    try {
        ...
        await sbAdmin.from('apex_tasks')
            .update({ status: 'completed', updated_at: new Date().toISOString() })
            .eq('id', taskId);
        await _appendNotif(`✅ ${taskId} completed: ${task.title}`, 'success', humanId);
