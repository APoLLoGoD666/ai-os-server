...
const ABANDONED_TTL_MS         = 30 * 60 * 1000;  // 30 min inactive → ABANDONED
const DISTINCT_GOAL_THRESHOLD  = 0.15;             // below this overlap → new goal, not continuation
...
function _newThread(sessionId, { goal = '', execution_class = 'EXECUTIVE', priority = 0.5 } = {}) {
    return {
        ...
        is_strategic:           null, // cached by EAE on first evaluation — avoids repeated regex
        lastReconsideredAt:     null, // set by EAE reconsideration — never used for urgency scoring
        reconsiderationCount:   0,    // how many times EAE has reconsidered this thread
        reconsiderationBoost:   0,    // temporary score boost applied by EAE; cleared on focus promotion
    };
}
...
function _upsertThread(thread) {
    ...
    // Evict BEFORE pushing so the new thread is never itself the eviction candidate
    if (threads.length >= MAX_THREADS_PER_SESSION) {
        // Preference order: completed/abandoned → deferred → lowest-priority active (oldest first among ties)
        let evict = threads.findIndex(t => t.status === STATUS.COMPLETED || t.status === STATUS.ABANDONED);
        if (evict < 0) evict = threads.findIndex(t => t.status === STATUS.DEFERRED);
        if (evict < 0) {
            let minP = Infinity, minIdx = 0;
            for (let i = 0; i < threads.length; i++) {
                if ((threads[i].priority || 0.5) < minP) { minP = threads[i].priority || 0.5; minIdx = i; }
            }
            evict = minIdx;
        }
        threads.splice(evict, 1);
    }
    threads.push(thread);
}
...
const _compressing = new Set(); // per-thread mutex: prevents double-compress on concurrent async callers
...
const _RESUME_PATTERNS = /\b(continue|pick up|resume|go back|back to|earlier|where (were|did) (we|you)|what were we|where we left|finish(ing)?|what (about|happened to)|you (started|mentioned|were|said)|still working|get back to|come back to|follow up|revisit|return to)\b/i;

function _isExplicitResume(userMessage) {
    return _RESUME_PATTERNS.test(userMessage);
}

// ── Short tech-word allow-list ────────────────────────────────────────────────
// 3-letter acronyms the length filter would otherwise discard but that carry signal

const _SHORT_TECH_WORDS = new Set([
    'jwt', 'sql', 'k8s', 'api', 'css', 'aws', 'gcp', 'pod', 'cdn', 'dns',
    'ssl', 'tls', 'ssh', 'git', 'npm', 'cli', 'sdk', 'iam', 'vpc', 'rds',
    'sqs', 'sns', 'eks', 'ecr', 'ecs', 'iac', 'orm', 'ide', 'xml', 'mcp',
]);

// ── Synonym normalization — maps surface words to canonical forms ─────────────
// Enables "the production outage" to match "kubernetes deployment failure" etc.

const _SYNONYM_GROUPS = [
    ['deploy',      ['deployment', 'push', 'release', 'ship', 'rollout', 'publish', 'shipping']],
    ['kubernetes',  ['k8s', 'kube', 'container', 'containers', 'pod', 'pods', 'cluster', 'helm']],
    ['outage',      ['incident', 'down', 'crash', 'failure', 'broken', 'error', 'offline']],
    ['database',    ['db', 'postgres', 'sql', 'supabase', 'schema', 'migration', 'table', 'query']],
    ['performance', ['latency', 'slow', 'speed', 'fast', 'benchmark', 'optimize', 'optimise']],
    ['auth',        ['login', 'token', 'jwt', 'session', 'password', 'authentication', 'credential']],
    ['api',         ['endpoint', 'route', 'request', 'response', 'webhook', 'routes', 'endpoints']],
    ['server',      ['backend', 'express', 'node', 'render', 'service', 'services']],
    ['build',       ['compile', 'bundle', 'webpack', 'build', 'builds', 'building']],
    ['problem',     ['problem', 'trouble', 'concern', 'issue', 'issues', 'bug', 'bugs', 'fail', 'fails', 'failing']],
    ['production',  ['prod', 'live', 'staging', 'environment', 'infra', 'infrastructure']],
    ['test',        ['testing', 'tests', 'spec', 'specs', 'verify', 'validate', 'check']],
];

const _WORD_TO_CANONICAL = new Map();
for (const [canon, synonyms] of _SYNONYM_GROUPS) {
    _WORD_TO_CANONICAL.set(canon, canon);
    for (const s of synonyms) _WORD_TO_CANONICAL.set(s, canon);
}

function _canonical(w) { return _WORD_TO_CANONICAL.get(w) || w; }

function _scoreRelevance(thread, userMessage) {
    if (!userMessage || !thread.goal) return 0;
    const _keep = w => w.length > 3 || _SHORT_TECH_WORDS.has(w);
    const msgWords = new Set(
        userMessage.toLowerCase().split(/\W+/).filter(_keep).map(_canonical)
    );
    const goalWords = thread.goal.toLowerCase().split(/\W+/).filter(_keep).map(_canonical);
    if (!goalWords.length || !msgWords.size) return 0;
    const overlap = goalWords.filter(w => msgWords.has(w)).length;
    const base = overlap / Math.max(goalWords.length, msgWords.size);
    // Indirect resume bonus: ≥2 matching canonical tokens is a strong implicit signal
    const indirectBonus = overlap >= 2 ? 0.15 : 0;
    return base + indirectBonus;
}

// ── Thread resolution guard ───────────────────────────────────────────────────

function _isThreadResolved(thread, intent, mode, reply) {
    if (intent !== 'SIMPLE_QUERY' && mode !== 'REFLEX' && (reply || '').length >= 80) return false;
    // pending_actions is maintained by TOOL_DISPATCHED/TOOL_COMPLETED events
    if ((thread.pending_actions || []).length > 0) return false;
    if ((thread.confidence || 0.5) < 0.6)         return false;
    return true;
}
 ...
 */
// Intent → initial priority mapping: intent strength informs scoring
function _intentPriority(intent) {
    if (intent === 'MULTI_STEP_TASK') return 0.70;
    if (intent === 'TOOL_REQUIRED')   return 0.65;
    if (intent === 'CLARIFICATION')   return 0.40;
    return 0.50;
}
...
function updateFromResponse({ sessionId, intent, userMessage, reply, mode, executionClass }) {
    ...
    if (!active) {
        const isResolvedEarly =
            intent === 'SIMPLE_QUERY' ||
            mode   === 'REFLEX'       ||
            (reply || '').length < 80;
        if (isResolvedEarly) return; // trivial exchange — no thread needed
        const goal = _extractGoal(userMessage || '');
        createThread(sessionId, { goal, execution_class: executionClass || 'EXECUTIVE', priority: _intentPriority(intent) });
        active = _getThreads(sessionId).find(t => t.status === STATUS.ACTIVE);
        if (!active) return;
    } else {
        // Distinct-goal detection: if the new message is a substantive task with low content
        // overlap to the current thread, defer the current thread and open a new one.
        const isNewTask = (intent === 'MULTI_STEP_TASK' || intent === 'TOOL_REQUIRED')
            && mode !== 'REFLEX' && (reply || '').length >= 80;
        if (isNewTask && _scoreRelevance(active, userMessage) < DISTINCT_GOAL_THRESHOLD) {
            active.status = STATUS.DEFERRED;
            active.updated_at = Date.now();
            const goal = _extractGoal(userMessage || '');
            createThread(sessionId, { goal, execution_class: executionClass || 'EXECUTIVE', priority: _intentPriority(intent) });
            active = _getThreads(sessionId).find(t => t.status === STATUS.ACTIVE);
    ...
    const isResolved = _isThreadResolved(active, intent, mode, reply);
    if (isResolved) {
...
function stats(sessionId) {
    ...
    // Compute live counts from store to avoid counter drift
    let liveActive = 0, liveInterrupted = 0, liveBackground = 0, liveResumed = 0;
    for (const threads of _store.values()) {
        for (const t of threads) {
            if (t.status === STATUS.ACTIVE)      liveActive++;
            if (t.status === STATUS.INTERRUPTED) liveInterrupted++;
            if (t.status === STATUS.BACKGROUND)  liveBackground++;
            if (t.surfaced_to_user)              liveResumed++;
        }
    }
    const sorted = _resolutionTimes.slice().sort((a, b) => a - b);
    return {
        active_threads:      liveActive,
        interrupted_threads: liveInterrupted,
        resumed_threads:     liveResumed,
        background_threads:  liveBackground,
