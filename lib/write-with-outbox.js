 ...
 *
 * stateOp shape:  { op: string, args?: object }
 *   op   — whitelisted operation name recognised by write_outbox_with_state (CASE branch).
 *   args — jsonb parameters passed to the operation; each branch uses EXECUTE ... USING,
 *          no string concatenation. Unknown op names raise inside the transaction.
 *
 * Passing { sql: ... } is rejected at the JS layer (Phase 0 hardening: injection surface closed).
 *
 * @param {{ op: string, args?: object }|null} stateOp
...
async function writeWithOutbox(stateOp, event) {
    ...
    if (stateOp) {
        if ('sql' in stateOp) throw new Error(
            'writeWithOutbox: stateOp.sql is no longer accepted — ' +
            'use { op, args } (Phase 0 hardening: whitelisted-operation RPC, injection surface closed)'
        );
