...
async function computeCivilizationHealth() {
  ...
  // Persist snapshot (table created in migration 015)
  const { error } = await _sb().from('civilization_health_snapshots').insert({
  ...
  if (error) logger.warn('telemetry', 'snapshot write failed', { error: error.message });
...
async function _taskSuccessRate() {
  const { data } = await _sb().from('apex_agent_runs').select('status').gte('created_at', _daysAgo(7));
  if (!data?.length) return null;
  return data.filter(r => r.status === 'completed').length / data.length;
}

async function _avgDuration() {
  const { data } = await _sb().from('apex_agent_runs')
    .select('duration_ms').eq('status', 'completed').gte('created_at', _daysAgo(7));
  if (!data?.length) return null;
...
async function _failuresByCategory() {
  const { data } = await _sb().from('apex_agent_runs').select('task_category, status').gte('created_at', _daysAgo(7));
  ...
  for (const r of data) {
    ...
    map[cat].total++;
    if (r.status === 'failed') map[cat].failed++;
  }
  return map;
}
