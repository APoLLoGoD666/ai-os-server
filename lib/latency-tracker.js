...
const VALID_SPANS = new Set([
    'session_start',
    'audio_received',
    'transcript_available',     // inputTranscription.text arrived from Gemini
    'route_selected',           // routing decision made (gemini/haiku/sonnet)
    'memory_lookup_start',
    'memory_lookup_end',
    'claude_start',
    'claude_first_token',
    ...
    'tool_complete',
    'tts_start',
    'tts_chunk_start',          // per-chunk TTS call initiated
    'tts_chunk_complete',       // per-chunk TTS response received
    'first_audio',
    'first_meaningful_output',
    'gemini_audio_start',       // first Gemini-native PCM chunk forwarded to browser
    'cancel_audio_sent',        // cancel_audio message dispatched to browser (barge-in)
    'user_interrupted',
    'completed',
]);

// Lazy ref — populated on first mark() to avoid circular require at startup
let _registryRef = null;
...
class LatencyTracker {
    ...
    mark(sessionId, span, meta = {}) {
        ...
        // Attach registry cognitive snapshot if available — lazy require avoids circular init
        let cogSnap = null;
        try {
            if (!_registryRef) _registryRef = require('./session-state-registry');
            if (sessionId) cogSnap = _registryRef.getDerivedCognitiveSnapshot(sessionId);
        } catch(e) { console.debug('[LatencyTracker] cognitive snapshot unavailable:', e.message); }
        s.snapshots[span] = {
    ...
    endSession(sessionId, outcome = {}) {
        ...
        const base = s.spans.transcript_available || s.spans.audio_received || s.start;
        // Clamp to [0, 3600000ms] — discards impossible values caused by clock-source
        // mixing or stale timestamps. Clamped entries appear as null in stats (→ "—").
        const _lat = (span) => {
            if (span == null || span <= base) return null;
            const v = span - base;
            return v < 3_600_000 ? v : null;
        };
        s.ack_latency        = _lat(s.spans.first_audio);
        s.meaningful_latency = _lat(s.spans.first_meaningful_output);
        s.completion_latency = _lat(s.spans.completed);
...
function _spanStats(sessions) {
    const SPAN_NAMES = [
        'audio_received', 'transcript_available', 'route_selected',
        'memory_lookup_start', 'memory_lookup_end',
        'claude_start', 'claude_first_token', 'tool_dispatch', 'tool_complete',
        'tts_start', 'tts_chunk_start', 'tts_chunk_complete',
        'first_audio', 'first_meaningful_output',
        'gemini_audio_start', 'cancel_audio_sent',
