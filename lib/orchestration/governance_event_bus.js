...
function emit(event_type, payload) {
    ...
    // Stage 8: Bridge to main event bus for cross-bus observability
    setImmediate(() => {
        try {
            const _bus = require('../event-bus');
            _bus.emit('governance:' + event_type, { event_type, payload: entry.payload, seq: entry.seq, fingerprint: entry.fingerprint });
        } catch (_) {}
    });
}
