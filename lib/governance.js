...
async function issueCertification(taskId, traceId, commitSha, score, evidence) {
    try {
        ...
        const { data } = await _sb().from('certifications').insert({
            ...
            score, status: 'certified', evidence: evidence || null,
...
async function appendEvidenceBlock(payload) {
    try {
        ...
        const contentHash = _hash(JSON.stringify(payload));
...
async function onPipelineComplete(taskId, traceId, { graphId, spanId, conditionsMet = [], commitSha, costUsd, durationMs, complexity, agentLogs, spec, attempts }) {
    ...
    for (const log of (agentLogs || [])) {
        ...
        await recordExecutionNode(traceId, taskId, graphId, role, role, 'completed', dur, null, null, null, out);
    }
    ...
    // Domain 17 — cost entry
    const parsedCost = parseFloat(costUsd) || 0;
    await recordCostEntry(taskId, traceId, 'pipeline', null, parsedCost, null, null);
...
async function onPipelineFailed(taskId, traceId, { graphId, spanId, conditionsMet = [], error, spec, agentLogs, costUsd, durationMs }) {
    const cs = [...(conditionsMet || [])];
    ...
    // Domain 17 — cost entry (even on failure, cost was incurred)
    const parsedCost = parseFloat(costUsd) || 0;
    if (parsedCost > 0) {
        await recordCostEntry(taskId, traceId, 'pipeline', null, parsedCost, null, null);
        cs.push('cost_entry_recorded');
    ...
    await recordExecutionEdges(graphId, stages);
    for (const log of (agentLogs || [])) {
        await recordExecutionNode(traceId, taskId, graphId, log.role, log.role, 'failed', log.duration || 0, null, null, null, log.result || {});
    }
