...
// Canonical JSON — delegated to shared module (lib/canonical-json.js).
// Used for evidence payload hashing; stable across jsonb round-trips.
const _canonicalize = canonicalJson;

// Fire-and-forget write wrapper — Constitution Article 4.
// Every write asserts on the returned error object; non-duplicate failures alert to Slack.
// Never throws — governance writes must never crash the caller.
function _w(promise, ctx) {
    return promise.then(({ error } = {}) => {
        if (error) {
            const isDup = error.message?.includes('duplicate') || error.message?.includes('unique');
            if (!isDup) {
                _log.warn('governance', `write error: ${ctx}`, { error: error.message });
                try {
                    const { alertError } = require('../services/slack/slack-alerts');
                    alertError(`Governance write failure`, `${ctx}: ${error.message}`, 'Governance').catch(() => {});
                } catch (_) {}
            }
        }
    }).catch(e => {
        _log.warn('governance', `write exception: ${ctx}`, { error: e.message });
    });
}
...
async function startExecutionGraph(traceId, taskId, metadata) {
    try {
        const { data, error } = await _sb().from('execution_graphs').insert({
            trace_id: traceId, task_id: taskId, status: 'running',
            started_at: new Date().toISOString(), metadata: metadata || null,
        }).select('id').single();
        if (error) {
            const isDup = error.message?.includes('duplicate') || error.message?.includes('unique');
            if (!isDup) _log.warn('governance', 'write error: execution_graphs.insert', { error: error.message });
        }
        return data?.id || null;
    } catch { return null; }
}
...
async function completeExecutionGraph(graphId, stageCount, status = 'completed') {
    try {
        if (!graphId) return;
        await _w(_sb().from('execution_graphs').update({
            status, stage_count: stageCount, completed_at: new Date().toISOString(),
        }).eq('id', graphId), 'execution_graphs.update');
    } catch {}
}
...
async function recordExecutionNode(traceId, taskId, graphId, stage, agentRole, status, durationMs, model, tokensIn, tokensOut, outputSummary) {
    try {
        const outputHash = outputSummary ? _hash(_canonicalize(outputSummary)) : null;
        await _w(_sb().from('execution_nodes').insert({
...
async function recordExecutionEdges(graphId, stages) {
    try {
        ...
        if (edges.length) await _w(_sb().from('execution_edges').insert(edges), 'execution_edges.insert');
    } catch {}
}
...
async function recordSystemEvent(eventType, source, taskId, traceId, payload) {
    try {
        await _w(_sb().from('system_events').insert({
...
async function recordAgentDecision(traceId, taskId, stage, agentRole, decisionType, reasoning, confidence, inputs, outputs, model, tokensIn, tokensOut, durationMs) {
    try {
        await _w(_sb().from('agent_decisions').insert({
...
async function recordLessonSource(lessonId, taskId, traceId, runId, failureId, lessonType, context) {
    try {
        await _w(_sb().from('lesson_sources').insert({
...
async function recordRootCauseReport(taskId, traceId, failureEventId, rootCause, contributingFactors, evidence, recommendations) {
    try {
        await _w(_sb().from('root_cause_reports').insert({
...
async function recordHealingEvent(taskId, traceId, trigger, strategy, status) {
    try {
        const { data, error } = await _sb().from('healing_events').insert({
            task_id: taskId, trace_id: traceId, trigger, strategy, status: status || 'initiated',
        }).select('id').single();
        if (error) {
            const isDup = error.message?.includes('duplicate') || error.message?.includes('unique');
            if (!isDup) _log.warn('governance', 'write error: healing_events.insert', { error: error.message });
        }
        return data?.id || null;
    } catch { return null; }
}
...
async function recordDeploymentVerification(graphId, checkType, status, details) {
    try {
        await _w(_sb().from('deployment_verifications').insert({
            graph_id: graphId, check_type: checkType, status, details: details || null,
        }), 'deployment_verifications.insert');
    } catch {}
}
...
async function issueCertification(taskId, traceId, commitSha, score, evidence) {
    try {
        ...
        const { data, error } = await _sb().from('certifications').insert({
        ...
        if (error) {
            const isDup = error.message?.includes('duplicate') || error.message?.includes('unique');
            if (!isDup) _log.warn('governance', 'write error: certifications.insert', { error: error.message });
        }
        return data?.id || null;
    } catch { return null; }
}
...
async function revokeCertification(certId, reason, evidence) {
    try {
        await _w(_sb().from('certifications').update({
            status: 'revoked', revoked_at: new Date().toISOString(),
            revocation_reason: reason || null,
        }).eq('id', certId), 'certifications.update');
    } catch {}
}
...
async function recordEvidenceHash(entityType, entityId, value) {
    try {
        ...
        await _w(_sb().from('evidence_hashes').insert({
...
async function appendEvidenceBlock(payload, chainId = 'main') {
    try {
        const sb = _sb();
        // Get last block in the specified chain
        ...
        await _w(sb.from('evidence_blocks').insert({
            chain_id: chainId, sequence: seq,
...
async function startOtelSpan(traceId, parentSpanId, name, kind, attributes) {
    ...
    try {
        await _w(_sb().from('otel_spans').insert({
...
async function endOtelSpan(spanId, status, durationMs, events) {
    try {
        await _w(_sb().from('otel_spans').update({
            status: status || 'OK',
            end_time: new Date().toISOString(),
            duration_ms: durationMs || null,
            events: events || null,
        }).eq('span_id', spanId), 'otel_spans.update');
    } catch {}
}
...
async function recordCostEntry(taskId, traceId, stage, model, costUsd, tokensIn, tokensOut) {
    try {
        await _w(_sb().from('cost_accounting').insert({
...
async function recordQualityScore(taskId, traceId, dimension, score, evidence) {
    try {
        await _w(_sb().from('quality_scores').insert({
...
async function recordRiskScore(taskId, traceId, riskType, score, factors) {
    try {
        await _w(_sb().from('risk_scores').insert({
...
async function createIncident(taskId, traceId, severity, title, description) {
    try {
        const { data, error } = await _sb().from('incidents').insert({
            task_id: taskId, trace_id: traceId, severity: severity || 'low',
            title, description: description || null, status: 'open',
        }).select('id').single();
        if (error) {
            const isDup = error.message?.includes('duplicate') || error.message?.includes('unique');
            if (!isDup) _log.warn('governance', 'write error: incidents.insert', { error: error.message });
        }
        return data?.id || null;
    } catch { return null; }
}
...
async function detectAnomaly(taskId, traceId, dimension, expected, actual) {
    try {
        ...
        await _w(_sb().from('anomalies').insert({
...
async function recordSloMeasurement(sloName, taskId, value) {
    try {
        ...
        await _w(sb.from('slo_measurements').insert({
            slo_id: slo.id, task_id: taskId || null, value, met,
        }), 'slo_measurements.insert');
        if (!met) {
            await _w(sb.from('slo_violations').insert({
                slo_id: slo.id, task_id: taskId || null, actual_value: value,
            }), 'slo_violations.insert');
        }
    } catch {}
}
...
async function recordSbomEntry(taskId, traceId, componentName, version, license, source) {
    try {
        await _w(_sb().from('sbom_entries').insert({
...
async function recordSimulation(taskId, traceId, simulationType, result, confidence) {
    try {
        await _w(_sb().from('simulations').insert({
...
async function captureDashboardSnapshot(data) {
    try {
        await _w(_sb().from('dashboard_snapshots').insert({
            snapshot_type: 'governance', data,
        }), 'dashboard_snapshots.insert');
    } catch {}
}
...
async function captureStateSnapshot(taskId, traceId, stage, state) {
    try {
        await _w(_sb().from('state_snapshots').insert({
...
async function captureEnvironmentSnapshot(taskId, traceId) {
    try {
        ...
        await _w(_sb().from('environment_snapshots').insert({
            task_id: taskId, trace_id: traceId,
...
async function issueSystemCertification(taskId, traceId, conditionsMet) {
    try {
        ...
        await _w(_sb().from('system_certifications').insert({
...
async function onPipelineComplete(taskId, traceId, { graphId, spanId, conditionsMet = [], commitSha, costUsd, durationMs, complexity, agentLogs, spec, attempts, agentTokens }) {
    ...
    // Domain 23 — SLO measurements
    await recordSloMeasurement('pipeline_success_rate', taskId, 1.0);
