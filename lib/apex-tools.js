...
async function toolGetNotifications(unreadOnly = true, humanId = null) {
    try {
        // Read from apex_notifications (current table) with per-account scoping
        let q = sbAdmin.from('apex_notifications')
        ...
        const summary = items.map(n =>
            `[${(n.type || 'info').toUpperCase()}] ${n.message}`
        ).join('\n');
        items.forEach(n => sbAdmin.from('apex_notifications').update({ read: true }).eq('id', n.id).catch(() => {}));
...
async function toolListTasks(humanId = null) {
    try {
        let q = sbAdmin.from('apex_tasks')
            .select('id,title,status,created_at')
            .not('status', 'in', '("completed","done","rejected")')
            .order('created_at', { ascending: false })
            .limit(20);
        if (humanId) q = q.or(`human_id.eq.${humanId},human_id.is.null`);
        ...
        if (error) throw new Error(error.message);
        const tasks = data || [];
        if (!tasks.length) return { tasks: [], summary: 'No pending tasks.' };
        const summary = tasks.map((t, i) => `${i + 1}. [${t.status}] ${t.title}`).join('\n');
...
async function toolGetCalendarEvents(days = 7, humanId = null) {
    try {
...
async function toolGetFinanceSummary(humanId = null) {
    try {
        const _hid = humanId;
        const _scope = (q) => _hid ? q.or(`human_id.eq.${_hid},human_id.is.null`) : q;
        const [entries, subscriptions] = await Promise.all([
            _scope(sbAdmin.from('apex_finance_entries').select('type,amount,category,description,transaction_date').order('transaction_date', { ascending: false }).limit(15)),
            _scope(sbAdmin.from('apex_subscriptions').select('name,amount,billing_cycle').eq('active', true).limit(10)),
        ]);
        ...
        if (rows.length) {
            parts.push(`Recent: £${income.toFixed(2)} income, £${expense.toFixed(2)} expenses`);
            const top3 = rows.slice(0, 3).map(r => `${r.description || r.category} £${Number(r.amount).toFixed(2)}`).join(', ');
            parts.push(`Latest: ${top3}`);
        }
        ...
        return { summary, entries: rows, subscriptions: subscriptions.data || [] };
    } catch (e) {
        return { error: e.message };
    }
}

async function toolGetHealthSummary(humanId = null) {
    try {
        const today = new Date().toISOString().split('T')[0];
        const week  = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
        const _scope = (q) => humanId ? q.or(`human_id.eq.${humanId},human_id.is.null`) : q;
        const [workouts, nutrition, sleep, mood] = await Promise.all([
            _scope(sbAdmin.from('apex_workouts').select('type,duration_minutes,notes,workout_date').gte('workout_date', week).order('workout_date', { ascending: false })).limit(5),
            _scope(sbAdmin.from('apex_nutrition_log').select('food_name,calories,protein_g,carbs_g,fat_g,meal_type').eq('log_date', today)).limit(100),
            _scope(sbAdmin.from('apex_sleep_log').select('sleep_date,duration_hours,quality,notes').order('sleep_date', { ascending: false })).limit(3),
            _scope(sbAdmin.from('apex_mood_log').select('score,notes,logged_at').order('logged_at', { ascending: false })).limit(1),
        ]);
        const parts = [];
        if (workouts.data?.length) {
            const w0 = workouts.data[0];
            parts.push(`${workouts.data.length} workout${workouts.data.length !== 1 ? 's' : ''} this week — latest: ${w0.type} (${w0.duration_minutes}min)`);
        } else {
        ...
        if (sleep.data?.length) {
            ...
            parts.push(`Last sleep: ${s.duration_hours}h${s.quality ? ', quality ' + s.quality + '/10' : ''}`);
        }
        if (mood.data?.length) {
...
async function toolLogWorkout(data, humanId = null) {
    try {
        const today = new Date().toISOString().split('T')[0];
        const row = {
            type:             data.type || 'General',
            duration_minutes: Number(data.duration_minutes) || null,
            notes:            data.notes || null,
            workout_date:     data.workout_date || today,
            human_id:         humanId || null,
        };
        if (!row.type) return { ok: false, error: 'type is required' };
        const { data: result, error } = await sbAdmin.from('apex_workouts').insert(row).select().single();
        if (error) throw new Error(error.message);
        return { ok: true, id: result.id, workout_date: result.workout_date };
    } catch (e) {
        return { ok: false, error: e.message };
    }
}

async function toolLogMood(score, notes, humanId = null) {
    try {
        const s = Math.min(10, Math.max(1, Number(score)));
        if (isNaN(s)) return { ok: false, error: 'score must be 1–10' };
        const { data: result, error } = await sbAdmin.from('apex_mood_log')
            .insert({ score: s, notes: notes || null, human_id: humanId || null }).select().single();
        if (error) throw new Error(error.message);
...
async function toolLogSleep(data, humanId = null) {
    try {
        const today = new Date().toISOString().split('T')[0];
        const row = {
            sleep_date:     data.sleep_date || today,
            bedtime:        data.bedtime || null,
            wake_time:      data.wake_time || null,
            quality:        data.quality ? Math.min(10, Math.max(1, Number(data.quality))) : null,
            duration_hours: data.duration_hours ? Number(data.duration_hours) : null,
            notes:          data.notes || null,
            human_id:       humanId || null,
        };
        const { data: result, error } = await sbAdmin.from('apex_sleep_log').insert(row).select().single();
        if (error) throw new Error(error.message);
        return { ok: true, id: result.id, sleep_date: result.sleep_date };
    } catch (e) {
        return { ok: false, error: e.message };
    }
}

async function toolGetRelationshipSummary(humanId = null) {
    try {
        const today = new Date().toISOString().split('T')[0];
        const week  = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
        const _scope = (q) => humanId ? q.or(`human_id.eq.${humanId},human_id.is.null`) : q;
        const [people, overdue, recent] = await Promise.all([
            _scope(sbAdmin.from('apex_people').select('id', { count: 'exact', head: true })),
            _scope(sbAdmin.from('apex_follow_ups').select('note,due_date').eq('completed', false).lt('due_date', today).order('due_date', { ascending: true })).limit(5),
            _scope(sbAdmin.from('apex_interactions').select('id', { count: 'exact', head: true }).gte('interaction_date', week)),
        ]);
        const parts = [];
...
const APEX_TOOLS = [
    ...
    {
        name: 'log_workout',
        description: 'Log a workout to the health tracker. Use when asked to log, record, or save a workout, gym session, run, walk, or any exercise.',
        input_schema: {
            type: 'object',
            properties: {
                type:             { type: 'string',  description: 'Workout type, e.g. "back and biceps", "running", "yoga".' },
                duration_minutes: { type: 'integer', description: 'Duration in minutes.' },
                notes:            { type: 'string',  description: 'Optional notes about the session.' },
                workout_date:     { type: 'string',  description: 'Date in YYYY-MM-DD. Defaults to today if omitted.' },
            },
            required: ['type']
        }
    },
    {
        name: 'log_mood',
        description: 'Log a mood score. Use when the user rates their mood or emotional state (e.g. "feeling great — 8/10", "mood is low today").',
        input_schema: {
            type: 'object',
            properties: {
                score: { type: 'integer', description: 'Mood score 1–10.' },
                notes: { type: 'string',  description: 'Optional mood note.' },
            },
            required: ['score']
        }
    },
    {
        name: 'log_sleep',
        description: 'Log a sleep record. Use when the user reports sleep duration, quality, or bedtime/wake time.',
        input_schema: {
            type: 'object',
            properties: {
                sleep_date:     { type: 'string',  description: 'Date of sleep in YYYY-MM-DD. Defaults to today.' },
                bedtime:        { type: 'string',  description: 'Bedtime in HH:MM 24h format, e.g. "23:30".' },
                wake_time:      { type: 'string',  description: 'Wake time in HH:MM 24h format, e.g. "07:00".' },
                duration_hours: { type: 'number',  description: 'Total sleep hours, e.g. 7.5.' },
                quality:        { type: 'integer', description: 'Sleep quality 1–10.' },
...
async function executeApexTool(name, input, { humanId = null } = {}) {
    if (name === 'web_search') {
    ...
    if (name === 'get_notifications') return await toolGetNotifications(input.unread_only !== false, humanId);
    ...
    if (name === 'list_tasks') return await toolListTasks(humanId);
    if (name === 'get_news') return await toolGetNews(input.category);
    if (name === 'get_calendar_events') return await toolGetCalendarEvents(input.days || 7, humanId);
    if (name === 'get_finance_summary') {
        const _fsResult = await toolGetFinanceSummary(humanId);
    ...
    if (name === 'get_health_summary') {
        const _hsResult = await toolGetHealthSummary(humanId);
    ...
    if (name === 'log_workout') return await toolLogWorkout(input, humanId);
    if (name === 'log_mood') return await toolLogMood(input.score, input.notes, humanId);
    if (name === 'log_sleep') return await toolLogSleep(input, humanId);
    if (name === 'get_relationship_summary') return await toolGetRelationshipSummary(humanId);
