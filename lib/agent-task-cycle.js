...
async function generateTaskCleanupProposal(task) {
    ...
    // V-11-H-B-C: propagate parent-task owner (agent_tasks.created_by) into
    // the agent_actions row so downstream reads honour ownership.
...
async function autoRunReadOnlyTaskSteps(taskId) {
    ...
    if (task.status === "completed") {
        ...
        if (standingApproval && execution.results.length) {
            // V-11-H-B-C: propagate parent-task owner into agent_actions row.
            await pgLogAgentAction(
...
async function runAgentPlanningCycle(taskId) {
    ...
    if (!parsed) {
        ...
        // V-11-H-B-C: propagate task owner (agent_tasks.created_by) so the
        // planning row lands with correct human_id, not NULL.
        await pgLogAgentAction(
            "agent_task_plan",
            "planned",
            task.goal,
    ...
    if (validation.fatalError) {
        ...
        // V-11-H-B-C: propagate task owner (agent_tasks.created_by).
        await pgLogAgentAction(
            "agent_task_plan",
            "failed",
            task.goal,
...
async function executeApprovedAgentTask(taskId, options = {}) {
    ...
    setImmediate(() => {
        pgInsertToolExecution({
            task_id:     taskId,
            agent_id:    task.assigned_agent_id || _TASK_CYCLE_AGENT_ID,
    ...
    if (finalStatus === "waiting_approval") {
        ...
        // Report significant completions to System agent for pipeline health tracking
        if (finalSteps && finalSteps.length > 2) {
            setImmediate(async () => {
                try {
                    const { invokeDomainAgent } = require('../agent-system/domain-agents');
                    await invokeDomainAgent('system',
                        `Pipeline completed: "${(task.goal || '').slice(0, 120)}" in ${finalSteps.length} steps (${(finalSkipped || []).length} skipped). Log any patterns worth optimizing.`,
...
async function getApprovedAgentActions(latestPlan) {
    let response;
    let _extToolsSection = '';
    try {
        const _reg = require('./tool-registry');
        const _schemas = await _reg.getConnectedSchemas();
        if (_schemas.length > 0) {
            const _schemaLines = _schemas.map(s => `  - ${s.name}: ${s.description}`).join('\n');
            _extToolsSection = `\n- external_tool  (use for any connected service below)\n  Connected tools:\n${_schemaLines}\n  Format: {"type":"external_tool","tool":"<name>","params":{...},"readOnly":<bool>}`;
        }
    } catch (_) {}
    try {
...
- list_documents
- list_files
- search_documents${_extToolsSection}
