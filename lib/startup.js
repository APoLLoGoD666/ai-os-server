...
function onListen(deps) {
    ...
    function _loadMastra() {
        try {
            ...
            // Use absolute MB threshold — percentage guards using heapTotal or even
            // heap_size_limit are unreliable on Render (reported limit varies by container).
            // 190MB is safe: production steady-state heap is 140-165MB; Mastra adds ~20MB.
            const heapUsedMb = mem.heapUsed / 1024 / 1024;
            if (heapUsedMb > 190) {
                console.warn(`[Mastra] load skipped — heap at ${heapUsedMb.toFixed(0)}MB (>190MB threshold). Retry in 10 min.`);
