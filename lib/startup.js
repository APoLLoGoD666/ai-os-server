...
function onListen(deps) {
    ...
    // Obsidian inbox watcher — converts dropped notes to awaiting_approval tasks (local only)
    setImmediate(() => {
        try { require('./inbox-watcher').start(); }
        catch (e) { console.warn('[InboxWatcher] start failed (non-fatal):', e.message); }
    });
