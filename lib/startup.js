...
function wireEvents({ _bus, sbAdmin, _agentQueue, _startAutoPipeline }) {
    if (process.env.REALITY_LOOP_ENABLED === 'true') {
        ...
        _bus.on(_bus.E.AGENT_COMPLETED, (event) => {
            const { task_id, elapsed_ms, ok } = event.payload || {};
            setImmediate(() => {
    ...
    _bus.on('civilization:opportunity:execute', (event) => {
        const { opportunityId, objective } = event.payload || {};
        if (!objective) return;
        const taskId = `CIV-OPP-${opportunityId || Date.now()}`;
        sbAdmin.from('apex_tasks')
            .insert({ id: taskId, title: String(objective).slice(0, 200), status: 'in_progress' })
...
function onListen(deps) {
    ...
    function _loadMastra() {
        try {
            ...
            // Use heapUsed vs heapLimit (not heapTotal) — heapTotal is V8's current
            // allocation and is always close to heapUsed, so heapUsed/heapTotal would
            // almost always exceed 0.75 even with plenty of headroom.
            const heapLimitMb = require('v8').getHeapStatistics().heap_size_limit / 1024 / 1024;
            const heapUsedMb  = mem.heapUsed / 1024 / 1024;
            const heapPct = heapUsedMb / heapLimitMb;
            if (heapPct > 0.75) {
                console.warn(`[Mastra] load skipped — heap at ${(heapPct * 100).toFixed(0)}% of limit (>75% threshold). Retry in 10 min.`);
    ...
    // Constitutional watchdog — initial tick + 30-min interval
    try {
        const _watchdog = require('./constitution/watchdog');
        _watchdog.start();
        _watchdog.tick().catch(() => {}); // populate _lastAssessment immediately so cert.lastResult is set before first request
        setInterval(() => _watchdog.tick().catch(() => {}), 30 * 60 * 1000);
        console.log('[Watchdog] Constitutional watchdog started (initial tick + 30-min interval)');
