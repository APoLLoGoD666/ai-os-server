...
function start() {
    ...
    setInterval(async () => {
        ...
        try {
            ...
            await sbAdmin.from('cron_run_log').delete().lt('started_at', cronCutoff);
            _log.info('retention', 'cron_run_log: purged records > 90 days');
        } catch (e) { _log.warn('retention', 'cron_run_log purge failed', { error: e.message }); }
    ...
    (function _scheduleCalendarSync() {
        const { syncGoogleCalendar } = require('./calendar/sync');
    ...
    // ── Self-Expansion — weekly gap scan (Sundays 10:00 UTC) ─────────────────
    (function _scheduleExpansionScan() {
        const now = new Date();
        const daysToSun = (7 - now.getUTCDay()) % 7 || 7;
        const next = new Date(Date.UTC(
            now.getUTCFullYear(), now.getUTCMonth(),
            now.getUTCDate() + daysToSun, 10, 0, 0
        ));
        setTimeout(() => {
            require('./cron-logger').wrapCron('expansion_gap_scan', () =>
                require('./expansion').runExpansionCycle()
            ).catch(() => {});
            setInterval(() => require('./cron-logger').wrapCron('expansion_gap_scan', () =>
                require('./expansion').runExpansionCycle()
            ).catch(() => {}), 7 * 24 * 60 * 60 * 1000);
        }, Math.max(next - now, 60_000));
        console.log(`[Expansion:Scan] Weekly scan scheduled for ${next.toISOString()}`);
    })();

    // ── Self-Expansion — monitoring graduation check (every 6h) ──────────────
    setTimeout(() => {
        require('./cron-logger').wrapCron('expansion_monitor', () =>
            require('./expansion').checkMonitoring()
        ).catch(() => {});
        setInterval(() => require('./cron-logger').wrapCron('expansion_monitor', () =>
            require('./expansion').checkMonitoring()
        ).catch(() => {}), 6 * 60 * 60 * 1000);
    }, 5 * 60 * 1000); // 5min after startup

    // ── Reality Architecture crons ────────────────────────────────────────────

    // Cron 1: Reality projection cycle — every 6h (90s after civilization runtime)
    (function _scheduleRealityProjection() {
        const _PROJECTORS = [
            './reality/projections/civilisation',
            './reality/projections/intelligence',
            './reality/projections/memory',
            './reality/projections/knowledge',
            './reality/projections/governance',
        ];
        const _runProjection = () => require('./cron-logger').wrapCron('reality_projection', async () => {
            await Promise.allSettled(_PROJECTORS.map(p => require(p).project()));
        }).catch(() => {});
        setTimeout(() => { _runProjection(); setInterval(_runProjection, 6 * 60 * 60 * 1000); }, 90_000);
        console.log('[RealityProjection] scheduled every 6h (90s delay)');
    })();

    // Cron 2: Reality health scoring — every 6h
    setInterval(() => require('./cron-logger').wrapCron('reality_health_score', () =>
        require('./reality/fabric').getSystemRealityHealth()
    ).catch(() => {}), 6 * 60 * 60 * 1000);

    // Cron 3: Understanding gap detection — daily at 03:00 UTC
    (function _scheduleUnderstandingGaps() {
        const _DOMAINS = ['civilisation','intelligence','registry','memory','infrastructure','observability','interface','knowledge','development','experiments'];
        const _run = () => require('./cron-logger').wrapCron('understanding_gaps', async () => {
            const und = require('./understanding');
            await Promise.allSettled(_DOMAINS.map(d => und.detectGaps(d, d)));
        }).catch(() => {});
        const _now = new Date();
        const _next = new Date(Date.UTC(_now.getUTCFullYear(), _now.getUTCMonth(), _now.getUTCDate() + (_now.getUTCHours() >= 3 ? 1 : 0), 3, 0, 0));
        setTimeout(() => { _run(); setInterval(_run, 24 * 60 * 60 * 1000); }, Math.max(_next - _now, 60_000));
        console.log(`[UnderstandingGaps] daily 03:00 UTC, next: ${_next.toISOString()}`);
    })();

    // Cron 4: Belief-Reality gap log — every 12h
    setInterval(() => require('./cron-logger').wrapCron('belief_reality_gap', async () => {
        const { logBeliefRealityGap } = require('./beliefs');
        const _DOMAINS = ['civilisation','intelligence','registry','memory','infrastructure','observability','interface','knowledge','development','experiments'];
        await Promise.allSettled(_DOMAINS.map(d => logBeliefRealityGap(d)));
    }).catch(() => {}), 12 * 60 * 60 * 1000);

    // Cron 5: Attention profile update — every 4h
    setInterval(() => require('./cron-logger').wrapCron('attention_profiles', () =>
        require('./attention/attention-manager').seedDomainAttentionProfiles()
    ).catch(() => {}), 4 * 60 * 60 * 1000);

    // Cron 6: Intent attribution audit — daily at 04:00 UTC (auto-close stale intents > 48h)
    (function _scheduleIntentAudit() {
        const _run = () => require('./cron-logger').wrapCron('intent_attribution_audit', async () => {
            const intent = require('./intent');
            const open = await intent.getOpenIntents(null, null);
            const stale = open.filter(i => Date.now() - new Date(i.created_at).getTime() > 48 * 60 * 60 * 1000);
            await Promise.allSettled(stale.map(i => intent.closeAttribution({ intentId: i.id, outcomeRef: null, outcomeMatched: null })));
        }).catch(() => {});
        const _now = new Date();
        const _next = new Date(Date.UTC(_now.getUTCFullYear(), _now.getUTCMonth(), _now.getUTCDate() + (_now.getUTCHours() >= 4 ? 1 : 0), 4, 0, 0));
        setTimeout(() => { _run(); setInterval(_run, 24 * 60 * 60 * 1000); }, Math.max(_next - _now, 60_000));
    })();

    // Cron 7: Meta-model weekly assessment — Sundays 06:00 UTC
    (function _scheduleMetaModelAssessment() {
        const _run = () => require('./cron-logger').wrapCron('meta_model_assessment', async () => {
            const mm     = require('./meta-model');
            const fabric = require('./reality/fabric');
            const health = await fabric.getSystemRealityHealth();
            const _ok    = health.filter(r => !r.error);
            const avg    = _ok.length ? Math.round(_ok.reduce((s, r) => s + r.composite, 0) / _ok.length) : 0;
            await mm.assessLayer({ layer: 'reality_fabric', dimension: 'completeness', qualityScore: avg, coverage: Math.min(100, avg * 1.1) });
            await mm.assessLayer({ layer: 'observer_infrastructure', dimension: 'completeness', qualityScore: avg * 0.9, coverage: avg * 0.8 });
        }).catch(() => {});
        const _now = new Date();
        const _days = (7 - _now.getUTCDay()) % 7 || 7;
        const _next = new Date(Date.UTC(_now.getUTCFullYear(), _now.getUTCMonth(), _now.getUTCDate() + _days, 6, 0, 0));
        setTimeout(() => { _run(); setInterval(_run, 7 * 24 * 60 * 60 * 1000); }, Math.max(_next - _now, 60_000));
        console.log(`[MetaModel] weekly assessment: ${_next.toISOString()}`);
    })();

    // Cron 8: Civilization self-model update — every 6h
    setInterval(() => require('./cron-logger').wrapCron('self_model_update', () =>
        require('./reality/self-model').evaluatePredictions()
    ).catch(() => {}), 6 * 60 * 60 * 1000);

    // Cron 9: Observer calibration audit — weekly Tuesdays 07:00 UTC
    (function _scheduleObserverCalibration() {
        const _run = () => require('./cron-logger').wrapCron('observer_calibration_audit', async () => {
            const obs = require('./observer-health');
            const sensors = await obs.listSensors();
            const overdue = sensors.filter(s => !s.last_calibrated || Date.now() - new Date(s.last_calibrated).getTime() > 7 * 24 * 60 * 60 * 1000);
            await Promise.allSettled(overdue.map(s => obs.recordCalibration({ sensorId: s.sensor_id, calibratedBy: 'calibration_cron', notes: 'auto-audit: overdue' })));
        }).catch(() => {});
        const _now = new Date();
        const _days = (9 - _now.getUTCDay()) % 7 || 7;
        const _next = new Date(Date.UTC(_now.getUTCFullYear(), _now.getUTCMonth(), _now.getUTCDate() + _days, 7, 0, 0));
        setTimeout(() => { _run(); setInterval(_run, 7 * 24 * 60 * 60 * 1000); }, Math.max(_next - _now, 60_000));
        console.log(`[ObserverCalibration] weekly Tuesday 07:00 UTC: ${_next.toISOString()}`);
    })();
}
