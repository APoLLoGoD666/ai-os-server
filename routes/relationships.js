...
router.get('/relationships/people', _auth, async (req, res) => {
    try {
        const { data, error } = await _scope(sb().from('apex_people')
            .select('*').order('name', { ascending: true }).limit(200), req);
...
router.get('/relationships/follow-ups', _auth, async (req, res) => {
    try {
        const { data, error } = await _scope(sb().from('apex_follow_ups')
            .select('*').eq('completed', false).order('due_date', { ascending: true }).limit(50), req);
        if (error) return res.status(500).json({ ok: false, error: error.message });
