...
router.get('/finance/invoices', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, invoices: [] });
...
router.get('/finance/expenses', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, expenses: [] });
...
router.get('/finance/subscriptions', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, subscriptions: [] });
...
router.get('/finance/investments', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, investments: [] });
...
router.get('/finance/balance', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, balance: 0, income: 0, expenses: 0 });
...
router.get('/finance/cashflow', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, cashflow: [] });
...
router.get('/finance/profit-loss', _auth, async (req, res) => {
    try {
        if (!isMasterRequest(req)) return res.json({ ok: true, pnl: [] });
        const cutoff = new Date();
        cutoff.setMonth(cutoff.getMonth() - 6);
        const { data, error } = await sb().from('apex_transactions')
